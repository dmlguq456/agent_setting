...
class WorkerRouteGuardTest(unittest.TestCase):
 def route(self):
  ...
  return R.compile_route("autopilot-code","dev","strong",ROOT,ROOT,predicates=ALL,signals=["shared-contract"],transport="headless",tracking="tracked",tracked_gate_evidence=gate)
 ...
 def test_source_commit_mismatch_rejected(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   route=R.compile_route("autopilot-code","dev","strong",repo,repo,signals=["shared-contract"],transport="headless",tracking="tracked",tracked_gate_evidence=gate); path=Path(td)/"route.json"; path.write_text(json.dumps(route))
