...
- **harness 강제**: `artifact-guard.sh` 가 체인을 기계 차단 — `spec/` 있는 프로젝트는 코드 편집 = `plans/` plan 전제, spec 작성 = research/analyze 전제. 작은 변경도 `autopilot-code --qa quick` 트레일. spec 없는 프로젝트는 코드 자유 (자동 scope). `/track` 우회.
