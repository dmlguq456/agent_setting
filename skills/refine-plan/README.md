...
> 본 README는 Notion 페이지 [✍️ refine-plan](https://www.notion.so/34987c2bb75381a5bc54e0f02e96b3e1)의 미러. `/sync-skills`로 양방향 동기화. 권위 있는 동작 명세는 `SKILL.md`.
...
## 호출 형식
```
/refine-plan <plan name or path> [--qa light|standard|thorough|adversarial]
```
...
3. 퍼지 검색 → `ls -d .claude_reports/plans/*$ARGUMENTS*`:
   - 1건 → `{match}/plan/plan.md` + `plan_ko.md`
   - 다건 → `_audit`/`_fix_` 없는 폴더 우선, 여전히 복수면 사용자 확인
   - 없음 → 에러

예: `/refine-plan inference-refactor` → `.../plan/plan.md`, `.../plan/plan_ko.md`
...
## QA Scaling
`$ARGUMENTS`의 `--qa` 명시 우선. 없으면 plan frontmatter의 `qa_level`. `--autonomy`는 strip만 (refine-plan은 autonomy gating 없음).
...
---
*원본: `~/.claude/skills/refine-plan/SKILL.md`*
