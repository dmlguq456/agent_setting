...
argument-hint: "<task description> [--mode paper|presentation|doc] [--intensity direct|quick|standard|strong|thorough|adversarial] [--qa quick|light|standard|thorough|adversarial] [--user-refine] [--no-clarify] [--from analyze|strategy|strategy-refine|draft|draft-refine|finalize]"
metadata:
  group: entry
  fam: doc
...
`<task description> [--mode paper|presentation|doc] [--intensity direct|quick|standard|strong|thorough|adversarial] [--qa quick|light|standard|thorough|adversarial] [--user-refine] [--no-clarify] [--from analyze|strategy|strategy-refine|draft|draft-refine|finalize]`
...
- `--qa`: 생략 시 선택 intensity 를 따른다. high-stakes 신호(신중히·camera-ready·submission 직전) 시 adversarial 자동 상향.
- `--user-refine`: 사용자가 명시적으로 pause 를 요청한 경우만 (orchestrator 가 자의로 추가 X).
- `--no-clarify`: Step 0 Scope Clarification skip.
- `--from <stage>`: 기존 artifact 폴더 path 또는 fuzzy short-name 을 positional 로 받아 해당 stage 에서 재개 — `pipeline_state.yaml` 에서 mode/qa_level/discovered_inputs/user_refine 복원, CLI flag 우선.
...
- 인자 해석·default·`--from` 재개·`pipeline_state.yaml`·input/output 구조: `references/invocation-and-args.md`.
- Pre-flight·Step 0·1·2·4·4b·5.5 의 orchestration 절차 (figure discovery/extraction, 해상도·crop 정책, path convention, 연구팀 draft 프롬프트, Tone/Conventions/Quality 임베드, KO mirror): `references/pipeline-steps.md`.
- Step 3(strategy review)·Step 5(draft review) 의 qa 별 reviewer 수·axis(A~D) 분해·quality reviewer/fact-checker 프롬프트: `references/review-and-qa.md`.
- Step 6 pipeline_summary 템플릿·terminal 보고·Safety Rules 전문: `references/summary-and-safety.md`.

## Reference Map

- `references/invocation-and-args.md`: Argument Parsing 전문(mode/auto-inference/input discovery/`--qa`/`--user-refine`/`--from`/format spec resolution), Decision Defaults 표, `pipeline_state.yaml` 스키마, Input Sources Convention, Artifact Structure.
- `references/pipeline-steps.md`: Pipeline Pre-flight~Step 2, Step 4 (4.0a figure discovery / 4.0b on-demand extraction / 4.0b-quality 해상도·crop 정책 / 4.0c path convention / 4.1 연구팀 draft[Tone Propagation·Mode-Specific Conventions·Quality Requirements 임베드] / 4-KO mirror), Step 4b factual detector, Step 5.5 editorial polish.
- `references/review-and-qa.md`: Step 3 Strategy Review + Step 5 Draft Review — qa 별 reviewer 수·axis 분해(A domain/content · B methodology/writing · C style · D cross-ref+coverage)·quality reviewer 프롬프트·fact-checker 프롬프트.
- `references/summary-and-safety.md`: Step 6 Pipeline Summary 템플릿(Process Log/Artifacts/Decision Points 포함) + Safety Rules 전문.
