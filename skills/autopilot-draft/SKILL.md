...
**Action**: 메인 에이전트가 mode-aware 2-4개 sharp question을 던진다. 사용자 답변을 task description에 통합 후 Step 1 진행. adapter pause/autonomy rule 적용(Claude Code: [CLAUDE.md](../../CLAUDE.md) §2) — 질문 던질 때 ScheduleWakeup 15-20분 동시 호출, 답 없으면 가장 가능성 높은 mode·길이·청중 default 로 자율 진행.
