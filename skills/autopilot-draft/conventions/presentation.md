...
본 §presentation 룰은 autopilot-draft `--mode presentation` (full deck / cheatsheet variant) + refine-doc / audit 으로 presentation artifact 수정·점검 시 모두 검사 적용.
...
8. **슬라이드 번호 정합성** (삽입 / 삭제 / 번호 변경 시 같은 edit pass 안에 모두 갱신):
   ...
   - (c) Changelog entry in frontmatter `changelog:` array (per `refine-doc` 컨벤션 — top-of-file HTML comment 금지, frontmatter 와 같이 있으면 markdown preview 깨짐)
