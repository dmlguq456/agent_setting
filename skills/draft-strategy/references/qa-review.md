...
- **Quality reviewer** (`editorial/review` unit): completeness, logical soundness, venue norms, and reviewer coverage for rebuttals
- **Fact-checker** (`research/fact-check` unit): verbatim comparison with in-artifact materials such as `analysis_project/paper/cards/*.md`, `analysis_project/doc/*/...`, and `research/{topic}/cards/*.md`; verify citation, venue, metric, year, and classification. [`roles/units/research/fact-check.md`](../../../roles/units/research/fact-check.md) is the single source for the eight-row classification table.
...
| **Adversarial** | External review imminent—camera-ready, submission, public report—or explicit `--intensity adversarial` | 2 parallel deep reviewers + 1 external adversary (the review unit dispatched to the codex harness via `stage-dispatch-fallback`) | **1 fast fact-checker** | 2 + 1 external |
...
2. **Check both verdicts:**
   ...
   - Quality 🔴 → re-dispatch the survey unit with quality findings, up to two rounds.
   - Fact-check 🔴 → re-dispatch the survey unit with mandatory reference grounding and reread the named cards/PDFs, up to two rounds.
   - Both → re-dispatch the survey unit with combined findings.
