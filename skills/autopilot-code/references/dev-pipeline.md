...
>
> **one-shot wait (SD-14)**: the conductor is a one-shot process — ending the turn ends the process, so a background completion notification never arrives. After dispatching a stage, do **not** end the turn on a Monitor/notification wait; poll in the same turn with `sh <agent-home>/utilities/dispatch-wait.sh --parent <conductor-slug>` (exit 0 = stage done → harvest; exit 2 = still alive → call it again; exit 3 = SUSPECT/DEAD → diagnose via dispatch-liveness + transcript tail, then re-dispatch, do not keep waiting). **After harvesting a stage, update its `.dispatch/jobs.log` row `open`→`done` in place before dispatching the next stage** — the registry duty (OPERATIONS §5.10); rows left open orphan the pipe for fleet/oncall and keep dispatch-wait's open-row count nonzero (2026-07-11 drill HARD-5 finding). `<agent-home>` = `$(utilities/agent-home.sh)`.
>
> **In-session fallback = closed three-condition set** (not an open escape hatch — SD-17): the in-session path is used **only when [a] intensity is `direct`/`quick` (micro-stages, no dispatch), [b] the runtime has no headless dispatch (codex/opencode `preflight.sh` reports unavailable), or [c] the conductor judges the work non-separable** — the stage artifact contract cannot carry it file-only because edits are boundary-coupled (a shared semantic anchor or boundary assertions couple them sequentially). A [c] judgment carries three duties: record the reasoning in `plans/<slug>/_internal/metrics.md` (**an unrecorded inline run is a contract violation** — the drill/audit surface), still parallelize the separable parts (census, disjoint file groups) with in-session workers, and use the explicit orchestrator opt-out (`STAGE_DISPATCH_INLINE_OK`) for dispatch-infra self-modification instead of self-judging. In all other `standard+` cases, dispatch is **mandatory, not optional** — the stage skills are the same contract either way.
>
> **usage-aware cross-harness dispatch (SD-16 — OPERATIONS §5.10 ⑧)**: stages are not pinned to Claude. Before dispatching, the conductor checks both harnesses' limit state deterministically with `sh <agent-home>/utilities/usage-check.sh` (per-harness `ok | limited(<reset>) | unknown`; a conservative jobs.log dead-limit-marker read — no scriptable official usage API exists as of 2026-07, so `ok` means "no known block", not a guarantee). Route **complementarily**: (a) **usage failover** — avoid a `limited` harness, route the stage to the one with headroom (Claude blocked → dispatch the stage via codex `preflight.sh dispatch`; and vice-versa), when both are free, prefer an explicit capacity override if set; otherwise keep the split neutral/balanced (the `bias <harness|auto>` line of usage-check — `HARNESS_CAPACITY_BIAS`, default `auto` since the 2026-07-13 runtime-currentness incident showed Codex/Claude quota assumptions change): bulk stages may ride an explicitly biased harness, while `auto` balances limit avoidance, fit, and spread without hardcoding runtime superiority; (b) **strength placement** — put a stage on the harness/model that fits it, and prefer to **cross the model family for the check stage** (a code-test/review worker on a different family than the code-execute maker — different failure modes = review diversity, the codex-review-team precedent). Initial conservative weighting = limit-avoidance > fit > spread (SD-OPEN-3). Cross-harness stage rows keep the same `--depth 2 --parent <conductor-slug> --worker-role …` shape and carry `harness=`/`owner_harness=`/`parent_sid` so the fleet view stays continuous across harnesses.
>
