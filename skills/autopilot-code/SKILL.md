...
name: autopilot-code
description: "Use when invoking the portable autopilot-code capability. 코드 작업 entry. spec 컨텍스트를 감지하고 plan→execute→test→report 흐름을 닫는다."
...
metadata:
  ...
  blurb: "코드 작업 entry. spec 컨텍스트를 감지하고 plan→execute→test→report 흐름을 닫는다."
---
