...
> **통합 기억 store 연동 (2026-06-15).** post-it 은 _프로젝트 단위 working tier_ 로서 통합 store([tools/memory](../../tools/memory/README.md))에 미러된다 — store SoT 는 `memory.db` (SQLite WAL), git 미러는 `dump.jsonl`. `recall`(=`mem recall`)이 post-it 내용까지 `memory.db` 한 면에서 검색하고, working lifecycle(만료·졸업)을 store 가 관할. post-it.md 는 여전히 _세션 시작 하네스 주입_ 의 1급 자리(이 파일 형식 유지). **`init` 으로 새 post-it.md 를 만들면 즉시 `python3 ~/.claude/tools/memory/mem.py register-postit <abs-path>` 로 레지스트리 등록** (NAS 재귀 스캔 회피 — store sync 가 직접 stat). sweep 의 시간 lifecycle 은 `mem lifecycle` 과 동류(시간 기반 working lifecycle)이되 임계값·동작이 다름 — post-it 은 ≥30d stale·≥90d archive 를 _플래깅_(사람-점검), store 는 `WORKING_TTL_DAYS`(현재 21d)로 _자동 만료_.
...
- **자동 메모리 시스템 대체 X** — post-it 은 store 의 _working tier 사람-편집 자리_ 이고(`mem recall` 한 면에서 검색), 하네스 auto-memory(`projects/*/memory/` → store durable mirror)와 역할이 다르다. post-it.md 파일 형식·세션 주입 1급성 유지(`/post-it` alias 유지 — spec D-2).
...
| `.claude_reports/post-it.md` (본 skill — store working cwd 면) | SessionEnd `mem sync` → `memory.db` **working**(project scope) 행으로 mirror; git 미러 `dump.jsonl` | 사용자 `/post-it` 로만 |

구분:
- **이 레포에만 적용되는 사실** (노션 위치, 데이터셋 경로, 진행 중 작업) → post-it.md
- **사용자 자신 / 일반 작업 선호** (Korean output, 코드 스타일) → durable auto-memory 또는 `--scope user`
겹치면 post-it.md 가 더 정확한 local context 라 우선. `mem recall` 이 양쪽을 한 면에서 검색한다.
