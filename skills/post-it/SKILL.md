...
| 경로 | store 위치 | 갱신 |
|---|---|---|
| `~/.claude/projects/*/memory/` (하네스 auto-memory write 면) | SessionEnd `mem sync` → store **durable**(project/global scope) mirror | 하네스 자동 write → sync |
| `.claude_reports/post-it.md` (본 skill — store working cwd 면) | SessionEnd `mem sync` → store **working**(project scope) mirror | 사용자 `/post-it` 로만 |
