---
name: analyze-user
description: "사용자 (Ui-Hyeop Shin) 의 cross-project 산출물 (paper / presentation / report / code / memory) 을 다단계로 스캔·분석해 `~/.claude/user_profile/*.md` 의 _범용 작업 성향_ 을 누적·갱신. autopilot-* 와 동급 ceremony — 사용자 프로필은 _한 번 만들어지면 모든 sub-agent 가 default 로 따르는 자료_ 라 작은 오류도 propagating. 따라서 source discovery → aspect 별 분석 → cross-aspect 일관성 검증 → 다중 QA gate → 산출 → pipeline summary 6 phase, `--qa standard` 가 default."
argument-hint: "<aspect> [--source <path>] [--mode init|update] [--qa quick|light|standard|thorough|adversarial] [--from discover|analyze|verify|qa|output|summary] [--user-refine]"
---
...
- `--qa`: **`standard` (default)**. _사용자 프로필은 propagating 자료_ 라 quick/light 권장 안 됨. _큰 갱신_ 또는 _init 모드_ 면 `thorough` 자동 상향. _첫 셋업_ 또는 _장기간 미갱신 후_ 는 `adversarial` 권장.
...
QA 5 단계 정의는 [CONVENTIONS.md §1](../../CONVENTIONS.md#1-qa-levels-canonical). 본 skill 적용:

- `quick` — 1-pass, single fact-check. **권장 안 함** (사용자 프로필 propagation 위험 큼). 짧은 incremental 갱신 한 자리만에 한정.
- `light` — 1-round, source coverage 점검만.
- **`standard` (default)** — 2-round, source coverage + pattern accuracy + cross-aspect consistency parallel review.
- `thorough` — 3-round, standard + factcheck (paper title / venue / 연도 / metric 수치 verbatim 대조).
- `adversarial` — thorough + Codex external review (`adversarial-review --scope auto`). _첫 셋업_ 또는 _장기 미갱신 후 통째 재구성_ 시.
...
### Phase 4 — Multi-agent QA Verification (`--qa <level>` 기반)

목적: 추출된 패턴이 _실제 source 와 일치_ 하는지, _사실 오류_ 없는지 검증. 사용자 프로필은 propagating 자료라 _여러 reviewer_ 병렬.

`--qa standard` (default) — 2 parallel reviewer:

- **Agent A — source coverage** (sonnet, `_internal/qa_coverage.md` 작성):
  ```
...
- **Agent B — pattern accuracy** (opus, `_internal/qa_accuracy.md` 작성):
  ```
...
`--qa thorough` — 3 parallel reviewer:

- 위 A·B 에 **Agent C — factcheck** (sonnet, `_internal/qa_factcheck.md`) 추가:
  ```
  paper 인용 verbatim 점검 — 제목 / 학회 / 연도 / 인용수 / DOI / arXiv ID.
  metric 수치 인용 점검 — paper / abstract 에서 직접 대조.
  ```

`--qa adversarial` — 위 + **Agent D — Codex external review** (codex-review-team, `_internal/qa_codex.md`):
  ```
  외부 hostile reader 관점 review — 사용자 프로필이 _과잉 일반화_ · _bias_ · _missing aspect_ 가 있는가?
  ```

산출:
- `_internal/qa_{coverage,accuracy,factcheck,codex}.md` (qa level 별 1-4 개)
...
2. **mode 별 처리**:
   - `init` — 기존 파일 `_internal/versions/v{N}/` 스냅샷 후 통째 교체.
   - `update` — 기존 파일 Read + draft 비교 + _누적 / 교체 / 제거_ 자리 결정 + Edit. changelog 한 줄 frontmatter 의 `changelog:` 배열에 추가.
...
```markdown
## {YYYY-MM-DD} — {aspect} {mode} (`--qa <level>`)
...
```
/analyze-user figure --source ~/nas/user/Uihyeop/doc/presentation/ --qa thorough
```
→ figure aspect, 추가 source 로 presentation 폴더 포함, 3-round QA.

```
/analyze-user all --mode init --qa adversarial
```
→ 모든 aspect 통째 재셋업, Codex 외부 review 포함 — 첫 셋업 또는 _장기 미갱신_ 시.

```
/analyze-user collab --qa light
```
→ 메모리 자료에서 collab aspect 만 가볍게 갱신.
...
- **첫 셋업** — `/analyze-user all --mode init --qa adversarial`. 본 사용자 자료 충분히 누적된 시점 (paper 5 편 이상) 에 한 번.
- **새 paper / 발표 / 보고서 직후** — 그 자료 추가만 incremental. `/analyze-user <relevant aspect> --qa standard`.
- **메모리 누적 자료 한 분기마다** — collab aspect 만 `/analyze-user collab --qa light`.
- **장기 미갱신 (6 개월+)** 후 — 전체 통째 재검증 `--qa thorough` 또는 `--qa adversarial`.
