...
argument-hint: "<task description> [--mode auto|app|library|api|cli|research (콤마로 다중)] [--qa quick|light|standard|thorough] [--user-refine]"
---
...
**Deployment diagram 의 자동 갱신** — ship 자리 변경 빈도 낮음 (provider 교체·env 추가·domain 연결 자리만). `autopilot-ship` 호출 자리에서 _deploy_record.md + Deployment diagram_ 한 묶음.
