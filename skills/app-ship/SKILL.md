---
name: app-ship
description: "[DEPRECATED — autopilot-spec 의 _setup mode_ 가 본 역할 흡수. 신규 호출 X]. 과거: App deployment setup — hosting (Vercel/Fly/Railway), CI/CD (GitHub Actions), env vars, domain. 현재: `/autopilot-spec` 호출 시 setup mode 자동 (specs/<name>/ + pipeline_state.yaml 존재 + 사용자 발화 'ship 셋업'·'배포 셋업' 신호)."
argument-hint: "<app name or path>  # DEPRECATED — /autopilot-spec setup mode 사용"
---

> **DEPRECATED (2026-05-25)** — autopilot-spec 의 setup mode 가 본 역할 흡수. 본 파일은 _레거시 참조_ 용. 실제 ship setup logic 은 `~/.claude/skills/autopilot-spec/SKILL.md` 의 `## Procedure → Mode B — setup` 안.
...
- `.claude_reports/specs/<name>/05_ship/deploy_record.md`
...
```
.claude_reports/specs/<name>/05_ship/ -- ✅ ship setup ready (provider: <name>)
```

배포까지 실행 완료 시:
```
.claude_reports/specs/<name>/05_ship/ -- ✅ deployed to <url>
```
