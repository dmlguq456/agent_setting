...
Agent(품질관리팀, mode=code-review):
  "Code review for app <name> build phase.
   ...
   Write review to .claude_reports/specs/<name>/04_qa/code_review.md"
```
...
Agent(품질관리팀, mode=test):
  "Graduated tests for app <name> build phase.
   ...
   Write report to .claude_reports/specs/<name>/04_qa/test_report.md"
```
...
Agent(디자인팀, mode=critic):
  "Visual review for app <name> after build.
   ...
   Write critique to .claude_reports/specs/<name>/04_qa/visual_qa.md"
```
...
- **issue 원인 분류** (back-jump 결정에 핵심):
  ...
  - **API contract 변경 필요** (백/프론트 양쪽 영향) → `/autopilot-spec --from spec` (spec back-jump, 하위 phase 자동 무효화)
  - **요구사항 자체 오류** (PRD 시나리오가 현실과 어긋남) → `/autopilot-spec --from spec`
  - **디자인 시스템 issue** (토큰·컴포넌트 변경 필요) → `/autopilot-spec --from design`
...
> Back-jump 흐름 상세: autopilot-spec SKILL.md 의 `## Mid-cycle Back-jump` 참조.
...
- `.claude_reports/specs/<name>/04_qa/code_review.md` — functional 정적 리뷰
- `.claude_reports/specs/<name>/04_qa/test_report.md` — graduated tests
- `.claude_reports/specs/<name>/04_qa/visual_qa.md` — UI critique (UI 있을 시)
- `.claude_reports/specs/<name>/04_qa/summary.md` — 종합 판정

## Return Format

통과:
```
.claude_reports/specs/<name>/04_qa/ -- ✅ QA passed (functional + visual)
```

실패:
```
.claude_reports/specs/<name>/04_qa/ -- 🔴 K issues found (M major) — see summary.md
```
