...
- `<artifact_path>` (REQUIRED): one of
  ...
  - Fuzzy short name (e.g., `se-seminar-tfrestormer`) — resolved via `ls -d .claude_reports/{plans,research,documents}/*$ARG* 2>/dev/null`. 1 match → use; multiple → ask user (글로벌 [CLAUDE.md](../../CLAUDE.md) §5 적용 — ScheduleWakeup 10분; 답 없으면 가장 최근 수정 artifact); 0 → error.
