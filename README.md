...
**통합 기억 — `~/.claude/memory/` (store, P10)** — 흩어졌던 단기(post-it)·장기(auto-memory)·프로필(user_profile) 3 면을 _하나의 포터블 store_ 로 통합 (Hermes 메모리 벤치마킹). git추적 markdown 원본 + SQLite FTS5 색인(파생). 레코드 = tier(working/durable) × scope(project/global) × type. **store 가 세션 주입의 source** — SessionStart hook `mem inject` 이 현 cwd 기억(단기+장기+사용자특성)을 자동 주입, SessionEnd `mem sync` 가 회수, `mem recall`(=`recall.sh`)이 store+세션 FTS 회상. 기억 _저장_ 은 자동(품질필터·dedup·injection 가드), _삭제·세팅변경_ 은 사람 게이트. 상세 → [`CONVENTIONS §7`](CONVENTIONS.md) · `tools/memory/`.
...
├── memory/                 통합 기억 store (markdown 원본 tier×scope + SQLite FTS5 색인) — 세션 주입 source (§7)
