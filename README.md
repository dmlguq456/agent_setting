...
| [`loops/README.md`](loops/README.md) | **상시 루프** (세션 밖 cron·headless) — **당직**(`scout`, 시간형 05:37 작업장 순찰·보고) · **서무**(`note`, 시간형 05:03 산출물 → worklog-board 노트화) · **모의훈련**(`golden/`, 사건형 — 지침 수정 후 행동 회귀 시험) · 후보 backlog. 출구는 모두 보고·제안까지 (merge 만 Claude 선별 책임 — CONVENTIONS §5.10) |
...
│
├── skills/                 작업 매뉴얼 — autopilot-* 파이프 + sub-skill + 운영 skill (SKILL.md 단일 출처)
├── agents/                 팀 8 — 근무시간의 부서 (개발·기획·품질관리·연구·편집·디자인·자료·codex리뷰)
├── agent-modes/            팀별 모드 페르소나 (dev / qa / research / editorial / design / material)
├── hooks/                  문지기 — 툴 호출 순간 강제 (artifact-guard 생성순서 · git-state-guard merge중 편집
│                           · spec-skill-gate + spec-read-marker prd 실독 게이트 · design-postwrite · herdr 상태)
├── loops/                  야간 업무 — 세션 밖 상시 루프: 당직(scout)·서무(note)·모의훈련(golden) + 후보 backlog
│
├── user_profile/           cross-project 사용자 성향 6 aspect (figure·writing·발표·분석·도메인·코딩)
├── tools/                  자체 도구 — design-mcp (시각 자가검증 MCP) · web-bundle
