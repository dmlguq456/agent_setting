...
> **3-tier 산출물 컨벤션** ([CONVENTIONS.md §7](CONVENTIONS.md#7-skill-output-convention-3-tier-t1t2t3)): T1 root = 메인 산출물 / T2 named subdir = 검토 자료 (`strategy/`, `cards/`, `dev_logs/` 등) / T3 `_internal/` = audit·raw·versions. 사용자는 보통 T1만 보면 됨.
...
| 모드 | 용도 | 명령 |
|---|---|---|
| `paper` | 학술 논문·camera-ready·major revision·기술 블로그·책 챕터 | `/autopilot-doc "<task>" --mode paper --user-refine` |
| `presentation` | 학회 발표·세미나·강의 (markdown only, PPTX X) | `/autopilot-doc "<task>" --mode presentation --user-refine` |
| `rebuttal` | 학회 reviewer 응답 | `/autopilot-doc "<task>" --mode rebuttal --user-refine` |
| `review` | reviewer 입장 peer review (template REQUIRED) | `/autopilot-doc "<task>" --mode review --user-refine` |
...
- **B → C** (manual): autopilot-code 결과(`plans/{...}/dev_logs/`)는 autopilot-doc이 _자동 인지 안 함_. `/analyze-project --mode doc plans/{...}` 1회로 doc 자료화 후 `report`/`paper` 모드 진행.
...
| `autopilot-doc` | 문서 strategy + draft (markdown). format spec은 `analysis_project/doc/{matching}/formats/`에서 자동 발견 (flag 없음) | `--mode rebuttal/paper/review/report/proposal/presentation` · `--qa` · `--from analyze/strategy/strategy-refine/draft/draft-refine/finalize` · `--user-refine` · `--no-clarify` |
...
- **`--qa quick/light/standard/thorough/adversarial`** — 리뷰 강도. **단일 정의는 [`CONVENTIONS.md`](CONVENTIONS.md) §1**에 있음 (5단계 reviewer 구성·fact-checker 적용 여부·skill별 max level 매트릭스·hard invariants). 본 줄은 요약:
  ...
  - `adversarial` = thorough + Codex 외부 리뷰 (camera-ready·grant 등). **autopilot-code · autopilot-refine 전용** — autopilot-doc/research는 thorough까지만
...
| 편집팀 (editorial-team) | opus | **사용자로 향하는 모든 문서의 점검 및 수정** — 옮기기 (영문 ↔ 국문, 필요 시만, 모드 A) / 다듬기 (판교체·번역체 회피, 표기 일관성, 가독성, 모드 B) / 점검만 (모드 C). 한국어든 영문이든 사용자가 직접 읽는 .md 산출물 전반. | autopilot-doc / autopilot-research / init-doc-strategy / init-plan 의 사용자 영역 단계 + autopilot-code 의 final-report + audit 보고서 + autopilot-refine 결과 + pipeline_summary | "문서 다듬어줘" / "판교체 정리" / "표기 통일" / "한국어 가독성 점검" / "국문 재서술" |
