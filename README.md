...
│
├── agents/                 팀 8 — 근무시간의 부서 (agent-modes/ 의 모드 페르소나와 짝)
...
├── hooks/                  문지기 — 툴 호출 순간 강제
│   ├── artifact-guard      신규 산출물 생성 순서 (spec←research·plan←spec·문서←research)
│   ├── git-state-guard     merge/rebase 중 편집 hard deny (golden g2 가 잡은 구멍)
...
│
├── loops/                  야간 업무 — 세션 밖 상시 루프 (호칭 = 회사 직무)
│   ├── scout.md·.sh        당직 — 매일 05:37 순찰 6항목, 보고서 notes/scout/ (파일 자체가 heartbeat)
│   ├── note.sh             서무 — 매일 05:03 autopilot-note 호출 래퍼 (전날 산출물 노트화)
│   ├── golden/             모의훈련 — 지침 수정 후 행동 회귀 시험
