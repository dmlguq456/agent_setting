...
| **🔬 연구·실험** (ML 실험·연구 코드) | `analyze-project`/`autopilot-research` → `autopilot-spec` (옵션, 뼈대·skeleton + ref 사전 점검만) → `autopilot-lab` (baseline 학습 → variation 실험 반복) | 자료 → 청사진·skeleton (ref ckpt _inference_ 검증) → baseline + variation 실험 (반복) |
...
flowchart LR
    ...
    subgraph EXP["🔬 연구·실험"]
        direction LR
        e1["analyze-project /<br/>autopilot-research"] --> e2["autopilot-spec<br/>(뼈대만, 옵션)<br/>↻ refine"] --> e3["autopilot-lab<br/>(baseline → variation)<br/>↻ 반복"]
    end
