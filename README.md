...
| "X 기능 새로 만들어줘" | autopilot-code dev 모드 (apps/ 자리면 앱 mode 자동 — 디자인팀 critic + DB 안전 + push 자동 deploy) |
...
/autopilot-app      "<app or setup description>" [--mode init|setup] [--qa ...] [--user-refine]
...
>
> - **연구개발** — `autopilot-research / analyze-project` → `autopilot-code` (반복)
> - **문서작업** — `autopilot-research / analyze-project` → `autopilot-draft` → `autopilot-refine` (반복)
> - **앱개발** — `autopilot-research / autopilot-app` → `autopilot-design` (옵션) → `autopilot-code` (앱 mode 자동, 반복) → autopilot-app `--mode setup` (ship 첫 setup·env·domain)
...
| [`autopilot-code`](skills/autopilot-code/SKILL.md) | 코드 작업 일반 (라이브러리·연구·앱 모두) — dev/debug. `apps/<name>/` 컨텍스트 자동 감지 → 앱 mode 활성화 (디자인팀 critic + DB 안전 + push 자동 deploy) |
| [`autopilot-app`](skills/autopilot-app/SKILL.md) | 앱의 _코드 외 결정·셋업_ — PRD + 스택 + scaffolding + skeleton + 보강 setup (ship 첫·env·domain·migration deploy). analyze-project 대칭 자리 |
| [`autopilot-draft`](skills/autopilot-draft/SKILL.md) | 문서 strategy + draft (paper/presentation/doc, markdown 만) |
| [`autopilot-design`](skills/autopilot-design/SKILL.md) | 시각 산출물 — UI/UX·슬라이드·다이어그램·아이콘 (init → refs → tokens → components → review → handoff). autopilot-app 의 디자인 사이클 또는 단독 호출 |
...
> - app 가족 (autopilot-app 안): `app-init` / `app-spec` (그 외 `app-build` / `app-qa` / `app-ship` / `app-iterate` 는 _DEPRECATED_ — autopilot-code 앱 mode 또는 autopilot-app setup mode 가 흡수)
...
|---|---|---|
| `autopilot-code` | "X 기능 만들어줘" / "X 디버그해봐" / "이 에러 고쳐줘" / 코드 변경 의도. _라이브러리·연구·앱 모두_. `apps/<name>/` 발견 시 _앱 mode 자동_ (디자인팀 critic + DB 안전 + push 자동 deploy) | `--mode dev/debug` 자동 추론 · `--qa thorough` (dev) / `standard` (debug) |
| `autopilot-app` | "X 앱 만들어줘" / "PRD 부터" (신규 init) · "Vercel 셋업·env 변경·domain 연결·migration 운영 배포" (보강 setup) | `--mode init/setup` 자동 감지 (`apps/<name>/pipeline_state.yaml` 부재 → init, 존재 → setup) · `--qa standard` |
