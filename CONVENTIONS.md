...
#### §5.4.3. 코드 트랙 = `spec/<project>/` (청사진) + `plans/<project>/` (작업) 2-bucket

> 2026-05-31 구조 개편: 이전엔 spec 있으면 `specs/<name>/` 안에 dev_log 까지 nesting, 없으면 `plans/<date>_<slug>/` 평면 — 이원 분기. **개편 후: 청사진은 항상 `spec/<project>/` 한 칸, 작업은 항상 `plans/<project>/<date>_<slug>/`.** 둘은 같은 `<project>` 이름으로 correlate 되는 형제 bucket. 숫자 prefix (00_/01_/02_/05_) 폐지 — 평이한 이름.

**`spec/<project>/`** — 프로젝트당 _한 개_ 청사진 (안정 layer):
...
**`plans/<project>/<date>_<slug>/`** — 작업 사이클 (반복 layer, 이전 dev_log + 이전 spec-less plans 통합):

```
plans/<project>/<date>_<slug>/
├── pipeline_summary.md   [T1]
...
|---|---|---|
| `autopilot-code` | spec 발견 자동 분기 + dev/debug mode + `--from plan/refine/execute/test/report` | `spec/<name>/pipeline_state.yaml` (있으면) |
| `autopilot-spec` | 신규 / refine v{N+1} — `--from spec/scaffold` 등 step 별 | `spec/<name>/pipeline_state.yaml` |
...
| `autopilot-design` | 신규 cycle / 재호출 — `--from init/refs/tokens/components/review/handoff` | `designs/<name>/design_state.yaml` 또는 `spec/<name>/design/design_state.yaml` |
...
| 감지 조건 | 처리 |
|---|---|
| `.claude_reports/spec/<name>/pipeline_state.yaml` 존재 | spec 자동 Read + 그 안 `mode` 배열 따라 _추가 logic_ 활성화. 산출 `plans/<name>/<date>_<slug>/` |
| 부재 (spec 없이 호출) | 일반 mode — cwd 단서 (`package.json` / framework) 만 보고 _경량 추론_. 산출 `plans/<project>/<date>_<slug>/` (project 추론 — cwd 폴더명) |
...
| `autopilot-spec` | `.claude_reports/spec/<name>/` (청사진 한 폴더 — `prd.md` 의 mode 별 섹션 + stack.md·design/·ship.md) |
| `autopilot-ship` | `.claude_reports/spec/<name>/ship.md` (배포 자료 누적, single source) + 프로젝트 root 의 `vercel.json` / `.github/workflows/deploy.yml` / `.env.example` (CI/CD·env 외부 자료, .claude_reports 밖) |
| `autopilot-design` (단독) | `.claude_reports/designs/<name>/` |
| `autopilot-design` (spec 위임) | `.claude_reports/spec/<name>/design/` |
| `autopilot-code` | `.claude_reports/plans/<name>/<date>_<slug>/` (spec 유무 무관 — 청사진은 `spec/<name>/`, 작업은 항상 `plans/<name>/`) |
