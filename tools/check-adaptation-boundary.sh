...
check_codex_tool_projection() {
  ...
  TOOL_DEFERRED="build-manifest.py check-adaptation-boundary.sh context-footprint.py adaptation-exemptions.tsv adaptation-guard.test.sh design-mcp web-bundle fleet profile install skill-conformance"
  tool_count=0
  for f in tools/*; do
    [ -e "$f" ] || continue
