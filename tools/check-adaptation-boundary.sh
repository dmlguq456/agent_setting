...
check_codex_tool_projection() {
  ...
  TOOL_DEFERRED="__pycache__ build-manifest.py generate.py harness_manifest.py generated-projections.test.sh check-adaptation-boundary.sh context-footprint.py adaptation-exemptions.tsv adaptation-guard.test.sh design-mcp skill-conformance web-bundle fleet profile install improvement"
  tool_count=0
  for f in tools/*; do
    [ -e "$f" ] || continue
