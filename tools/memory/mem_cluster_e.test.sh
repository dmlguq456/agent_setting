...
[ "$uv" = "3" ] && ok "①: PRAGMA user_version == 3 after migration" || bad "①: user_version=$uv (expected 3)"
...
# Verify tail columns are strength, last_accessed
tail_ok=$(echo "$fresh_cols" | python3 -c "import sys; s=sys.stdin.read().strip(); cols=s.split(','); ok = cols[-2]=='strength' and cols[-1]=='last_accessed'; print('ok' if ok else f'bad:{cols[-2:]}')")
[ "$tail_ok" = "ok" ] \
  && ok "①: column tail ends in ...,strength,last_accessed" \
...
echo "$inject_cleanup_7" | grep -q "정리 후보" \
  && ok "⑦: inject_cleanup_candidates surfaces near-dup from worktree cwd (project_key coherence)" \
  || bad "⑦: 정리 후보 not found from worktree (cleanup path not exercised)"
