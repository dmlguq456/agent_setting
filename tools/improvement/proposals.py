...
COLLECTOR_PROPOSAL_TARGETS = {"reproduced", "proposed"}
CONTEXT_FIELDS = {
    "source_revision",
    "source_dirty",
...
def _next_occurrence(record: Dict[str, Any]) -> int:
    value = record.get("occurrences", 1)
    if isinstance(value, bool) or not isinstance(value, int) or value < 1:
        raise ProposalError("invalid-record", "proposal occurrences must be a positive integer")
    return value + 1


def _has_human_decision(record: Dict[str, Any]) -> bool:
    history = record.get("history")
    if not isinstance(history, list):
        raise ProposalError("invalid-record", "proposal history must be a list")
    return any(
        isinstance(event, dict) and event.get("to") in APPROVAL_PROPOSAL_STATES
    ...
    with _mutation_lock(store) as root:
        if incident_key is not None:
            ...
            if matches:
                ...
                occurrence = _next_occurrence(record)
...
def transition(
    ...
    actor = _validate_actor(actor)
    collector = actor.startswith("loop:")
    ...
    with _mutation_lock(store) as root:
        ...
        if collector:
            if target not in COLLECTOR_PROPOSAL_TARGETS:
                raise ProposalError(
                    "collector-state-forbidden",
                    "named automated collectors may advance only to reproduced or proposed",
                )
            if _validate_incident_key(record.get("incident_key")) is None:
                raise ProposalError(
                    "incident-key-required",
                    "named automated collectors may transition only exact-key proposals",
                )
            if _has_human_decision(record):
                raise ProposalError(
                    "collector-state-forbidden",
                    "named automated collectors cannot change a human-reviewed proposal",
                )
        if target in APPROVAL_PROPOSAL_STATES:
            _approval(actor, approval_ref)
        decision_context = None
        decision_fingerprint = None
        if target == "reproduced" and collector:
        ...
        if target == "reproduced" and collector and decision_fingerprint:
