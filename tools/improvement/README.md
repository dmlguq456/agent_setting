...
Human review remains the only path to reviewed or terminal states.
Recurrence alone preserves the original base fingerprint. A named collector
must bind `reproduced` to a current `--context`; that explicit reproduction
is the only pre-review step that rebases the proposal for later freshness
checks.
