...
> - **사용자 지시·실제 세션 증거 (2026-07-23, v24)**: `codex-headless-context-parity` 이후 실제 Codex owner가 child 등록 뒤 5분 41초 동안 26개 후속 tool call을 남겼고, 새 parent-parking guard가 그중 21개 비대기 작업을 막아도 5개 wait continuation은 남았다. 사용자는 등록형 headless 적극 분사 전제를 유지하면서 모델이 잠들고 완료 때 한 번만 깨는 근본 수정을 지시했다.
> - **공식 Codex currentness (2026-07-23, v24)**: Codex CLI 0.145.0 App Server는 stable `thread/start`·`turn/start`와 streamed `turn/completed` lifecycle을 제공하고 같은 thread에 후속 turn을 시작할 수 있다. CLI 명령 자체는 experimental 표면이므로 runtime probe 실패 시 polling fallback을 명시하되 parity를 주장하지 않는다. MCP tool은 기본 `tool_timeout_sec=60`이라 사용자 runtime config 변경 없이 장기 join의 기본 경로로 채택하지 않는다.
> - **공식 Claude currentness (2026-07-23, v24)**: Claude Code 2.1.218은 `--session-id`, `--resume`, `--print --output-format stream-json` 및 background-agent supervisor를 노출한다. registered `claude -p` owner는 native subagent/agent-team으로 바꾸지 않고 같은 session을 bounded resume하는 adapter supervisor를 사용한다.
> - **사용자 관측·재현 (2026-07-24, v25)**: strong depth-2 review replica가 동시에 두 행으로 뜨지 않고 하나씩 순차 실행됐다. 정식 cross-harness drill에서 execute가 completion marker와 terminal row를 먼저 기록한 뒤 outer governor process/lease가 약 4.6초 더 생존했고, owner+draining predecessor+replica 한 개가 dispatch class cap 3을 채워 다른 replica admission이 75로 거부됐다. 별도로 terminal registry stage와 아직 살아 있는 runtime process가 Fleet의 `stage one-shot` 행으로 중복 투영됐다.
> - **코드 전수검토 (2026-07-24, v25)**: completion gate·runtime join·`dispatch-wait`·fallback watcher가 semantic marker/row terminal을 successor-ready와 동일시한다. 세 wrapper는 governor `check`와 실제 `run/acquire` 사이에 row claim을 끼워 TOCTOU를 만들며 OpenCode만 shared atomic spawn/PID identity를 우회한다. route의 strong plan은 의도적으로 순차이고, 병렬 대상은 `replica_group`으로 묶인 review sibling 두 leg뿐이다.
...
## 13.14 v24 — runtime-owned completion join (2026-07-23)

> SD-14는 Claude `-p` one-shot 제약의 우회책인 “같은 model turn에서
> `dispatch-wait` 반복”을 portable 기본값으로 승격했다. Codex adapter에는 완료
> event bridge 없이 해당 문구만 투영됐고, 2026-07-22 parity 검증은 byte-level
> transcript 격리만 측정해 parent model/tool 재진입을 검증하지 않았다. SD-78은
> 의사결정과 process scheduling을 분리해 이 전제를 교체한다.

### 13.14.1 SD-78 — exact batch join과 single-resume

- `standard+` registered-headless owner가 한 turn에서 자식 attempt를 등록하면 그
  turn은 polling tool을 호출하지 않고 runtime yield marker로 종료한다. owner process
  안의 **비모델 completion supervisor**가 canonical jobs registry에서
  `parent_attempt_id=<owner exact attempt>`인 current-contract direct child를 전부
  snapshot하고 terminal-ready까지 batch join한다. slug, cwd, 최근 transcript mtime,
  다른 owner의 row는 wake 근거가 아니다.
- join 중 parent model turn·tool call은 0이다. supervisor는 canonical liveness와
  exact terminal/marker 증거를 내부에서만 사용하며 raw child transcript, log,
  artifact body를 continuation에 넣지 않는다. 병렬 N개 중 하나가 먼저 끝나도 나머지
  N-1이 terminal-ready가 될 때까지 model을 깨우지 않는다.
- batch가 ready, timeout 또는 typed failure에 도달하면 supervisor는 같은 logical
  owner thread/session에 **bounded typed aggregate 한 번**만 전달한다. 모델은 exact
  harvest와 route gate를 수행하고 다음 stage를 분사하거나 최종 handoff를 낸다.
  terminal row가 여전히 open이면 exact harvest 외 local tool은 계속 deny한다.
- supervisor는 매 model turn 직전에 owner-attempt 전용 상태 파일에 이미 receipt로
  전달한 attempt 집합을 원자적으로 기록한다. open direct child가 있으나 그 집합과
  교집합이 없으면 아직 **batch registration phase**이므로 동일 parent slug에 결합된
  exact `dispatch-node --action start`만 추가로 허용한다. 첫 자식 등록 직후 모든 tool을
  막아 strong+의 두 번째 replica를 누락시키는 구현은 계약 위반이다. delivered open
  child가 하나라도 있으면 phase는 harvest-only이며 해당 delivered attempt의 exact
  harvest 외 도구를 모두 거부한다. 두 phase 모두 wait/liveness/raw inspection/복합
  shell/unrelated work를 거부하고, 상태가 missing·invalid이면 신규 dispatch 권한 없이
  recovery exact harvest만 허용한다. 정상 supervisor 종료 또는 비정상 종료를 감지한
  exact owner watcher가 상태 파일을 제거한다.
- intermediate Codex `turn/completed`나 Claude `result`는 worker final terminal event로
  노출하지 않는다. supervisor event namespace로 기록하고, owned child 0 + exact
  three-line handoff가 함께 성립한 마지막 turn만 기존 terminal inspector가 읽는
  `turn.completed`/`result` 한 건으로 정규화한다.
- Codex realization은 checked App Server stdio client가 한 thread에 여러
  `turn/start`를 보내고 `turn/completed`를 받는다. Claude realization은 한 UUID
  session의 첫 `claude -p --session-id`와 후속 `claude -p --resume`를 supervisor가
  직렬 구동한다. 둘 다 registered wrapper/Fleet/attempt/artifact 계약을 보존하며
  native subagent, agent team, background-agent row로 대체하지 않는다.
  Codex는 projected native PreToolUse bridge로 phase를 집행하고 Claude는 supervisor
...
- runtime probe가 supervisor 표면을 확인하지 못하면 기존 SD-14 polling은
  `completion_delivery=poll-fallback`으로만 허용한다. fallback은 parity/idle-success로
  보고하지 않으며 App Server/session-resume 실행 도중 실패한 attempt를 같은 wrapper가
  몰래 one-shot으로 재실행하지 않는다.
- scope는 registered-headless `standard+` owner다. `direct`, child가 없는 `quick`,
  depth-2 stage one-shot, 임의 detached stdout로 종료된 interactive TUI를 깨우는
  기능은 변경하지 않는다.

**acceptance**: ① 첫 exact child가 open인 undelivered phase에서 동일 parent의 두 번째
sibling start 허용, wait/read/git/harvest/foreign-parent start 거부 ② receipt 이후 delivered
open phase에서 exact harvest만 허용하고 신규 sibling/wait/unrelated tool 거부 ③ child
runtime window의 parent trace에 새 `turn/start`/model/tool event 0 ④ exact sibling 2개
동시 open fixture에서 둘 다 terminal-ready 뒤 continuation 1회 ⑤ foreign parent/stale/
malformed row wake 0, missing/invalid phase state에서 신규 dispatch 0 ⑥ intermediate
terminal suppression + final terminal 정확 1건, 이전 turn diagnostic bleed 0 ⑦
continuation payload는 attempt id·closed enum·bounded reason만, raw sentinel 0 ⑧ timeout/
protocol/runtime failure typed + silent rerun 0 ⑨ Codex App Server와 Claude
session-resume + command-scoped hook live smoke, unavailable fallback의 정직한 표기
⑩ 기존 harvest/liveness/Fleet/orphan/adaptation/portable guard 회귀 0.

## 13.15 v25 — successor readiness와 concurrent replica batch (2026-07-24)

### 13.15.1 SD-79 — semantic terminal과 execution quiescence 분리

- completion marker와 terminal jobs row는 stage의 **의미적 완료**를 즉시 기록한다.
  marker 작성·exact row close·Fleet stage done 표시는 늦추거나 되돌리지 않는다.
  다만 successor launch, completion join, `dispatch-wait`, fallback terminal watcher가
  내보내는 **successor-ready**는 `semantic_terminal AND exact_outer_process_quiescent`
  일 때만 참이다. marker만 보고 다음 model lease를 요청하는 경로는 금지한다.
- shared classifier는 attempt row의 outer governor identity를 단일 근거로 사용한다.
  exact PID/start가 live이면 `draining`, start mismatch·process 부재·zombie이면
  `quiescent`, current atomic-spawn schema에서 PID가 전혀 없는 closed row는
  명시적 `launch_outcome=never-launched` 증거가 있을 때만 quiescent다. PID가
  누락됐는데 그 증거가 없으면 current row도 `unverifiable`이다. namespace-local row는 host PID/start 증거로만
  quiescence를 확정하며 host identity가 없는 legacy row는 `unverifiable`로
  fail-closed한다. PID reuse는 replacement process를 신호하거나 기다리지 않는다.
- classifier는 completion marker gate, runtime-owned batch join, polling wait,
  ...
  terminal 관측을 캐시했더라도 process quiescence는 매번 재확인한다. timeout은 typed draining/unverifiable을
  보존하고 semantic success를 failure로 다시 쓰지 않는다. cap 상향, marker 지연,
  임의 sleep은 해법이 아니다. outer governor가 사라졌다는 증거는 그 process의
  `finally` lease release 또는 다음 locked state read의 stale-prune 이후이므로
  successor admission과 같은 경계를 이룬다.
- Codex·Claude stage wrapper는 current attempt의 local/host PID·start·process-group·
  PID namespace evidence를 shared atomic spawn primitive로 동형 기록한다. early
  governor denial/exit은 runtime log 부재와 무관하게 exact row를 typed terminal로
  닫는다. OpenCode는 governor reservation과 attempt env는 동형 적용하되, 현행
  parent-attempt binding·foreground lifecycle·supervisor snapshot 부재를 숨기지 않고
  registered standard+ depth-2 후보에서 `unsupported`로 제외한다. 완전 parity는 이
  세 축을 shared primitive로 이관한 뒤 별도 conformance로만 승격한다.

### 13.15.2 SD-80 — replica-group concurrent launch와 atomic admission

- `strong`은 plan 자체를 복제하지 않는다. portable DAG의 plan→plan-check→execute는
  순차이며, 병렬 보장은 immutable route의 같은 `replica_group`에 속한 정확히 두
  review sibling에만 적용한다. topology 변경과 lifecycle 버그 수정을 섞지 않는다.
- conductor는 replica 두 leg를 순차 `dispatch-chain` 호출로 열지 않고 한 번의
  checked non-model batch primitive에 넘긴다. batch는 route hash·jobs·cwd·exact
  parent attempt·dependency quiescence·group cardinality를 검증하고, 지원 증거가
  있으면 서로 다른 harness를 배정한 뒤 두 wrapper start를 동시에 실행한다.
  transient PID namespace에서는 두 foreground-scoped wrapper를 병렬 subprocess로
  유지하고, stable namespace에서는 detached lifecycle과 supervisor join을 보존한다.
- batch는 governor lock 한 번에서 replica N개 slot을 **원자 예약**한다. total/class/
  rolling-start budget 중 하나라도 N개를 수용하지 못하면 row와 model process를 0개
  만든다. 각 wrapper는 opaque reservation 하나를 exact governed process로 claim하며,
  미사용 reservation은 owner identity 소멸 또는 explicit cancel로 회수한다. 단일
  wrapper도 `check→row claim→run acquire` TOCTOU 대신 같은 reserve→claim 경로를 쓴다.
- 결과는 leg별 node/harness/attempt-id/typed launch state만 담은 bounded receipt다.
  raw child output은 포함하지 않는다. 한 leg의 post-reservation launch failure는 이미
  기동된 sibling을 죽이거나 중복 launch하지 않고, 실패 leg만 canonical SD-50 checked
  fallback 대상으로 남긴다. distinct supported harness가 하나뿐이면 동일-harness
  열화와 independence loss를 명시하며 silent serial/inline 하강은 금지한다.
- parent-parking guard는 exact same-parent replica batch start 한 건을 registration
  phase에서 허용하고, arbitrary shell composition·foreign parent·delivered phase batch는
  계속 거부한다. batch는 새 daemon/broker/model turn/dispatch depth를 만들지 않는다.

### 13.15.3 SD-81 — Fleet exact-attempt process projection

- runtime process/session scanner는 wrapper가 전달한 `AGENT_DISPATCH_ATTEMPT_ID`를
  `Session`/`DispatchJob`에 투영한다. OpenCode wrapper도 같은 env를 내보낸다.
  Fleet는 latest exact attempt의 terminal/draining registry index와 process row를
  exact join해 중복 `stage one-shot` process 행만 숨기고 route-node stage done 행은
  유지한다. cwd·slug·outer PID 단독 휴리스틱으로 unrelated session을 숨기지 않는다.
- registered depth-2 worker는 별도 context-window 실측을 주장하지 않는다. 기존
  subagent/session context 표면과 registered stage/Fleet lifecycle은 계속 분리한다.

**acceptance**: ① barrier fixture에서 marker/terminal row가 먼저 생겨도 outer governor가
live인 동안 gate·join·wait·fallback 모두 ready 0, 종료 직후 ready 1 ② PID reuse·zombie·
never-launched·namespace host-visible/unverifiable 판정이 동형이고 unrelated process signal
0 ③ owner 1 + replica 2를 cap 3에서 atomic admit하며 두 exact depth-2 row/process의
실제 overlap 관측, N-slot 부족 시 row/model spawn 0 ④ check/acquire TOCTOU 회귀 0,
Codex log 없는 governor exit도 terminal row로 수렴 ⑤ route replica cardinality·dependency·
parent binding 위반은 launch 0, strong plan 순차성은 그대로 ⑥ distinct supported harness
2개면 cross-harness 배치, 불가 시 typed independence degradation ⑦ Codex·Claude
stage lifecycle/PID/reservation parity, OpenCode reservation/env parity + standard+ depth-2
unsupported conformance ⑧ Fleet는 terminal stage 1행을 유지하고 같은
...
## 13.20 v30 — profile-sealed asymmetric N-way topology (2026-07-26)

### 13.20.1 SD-88 — 네 단계 실행 프로파일과 역할/프로파일 축 분리

- portable 실행 프로파일은 품질·지연·비용 의도를 나타내는 `deep`,
  `balanced-deep`, `light`, `mini` 네 단계다. model role은 작업 행동과 책임을,
  model profile은 concrete model tier와 effort/variant budget을 소유한다. 역할 이름으로
  topology의 실제 모델을 역추측하지 않는다.
- 기본 intent는 `deep = deep tier/xhigh`, `balanced-deep = deep tier/medium`,
  `light = light tier/medium`, `mini = mini tier/medium`이다. high/max/low는 route가
  별도로 봉인하거나 capacity/fallback 정책이 증거와 함께 선택하는 override이며 새
  기본 tier가 아니다. effort label은 vendor/model 간 절대 성능 척도가 아니므로 adapter가
  current runtime을 검증해 대응한다.
- 표준 owner는 `balanced-deep`, strong+ owner와 critical frame/plan/security anchor는
  `deep`을 사용한다. 일반 구현·정형 검토·test/report는 `light`가 기본이며 위험/불일치가
  확인될 때 `balanced-deep` 또는 `deep`으로 승급한다. depth 자체가 profile selector는
  아니고 node 의미·위험·intensity가 선택한다.
- registered depth-1/2 substantive topology는 `mini`를 fail-closed한다. mini는 lifecycle,
  분류, 짧은 의미 판정처럼 main이 소유하는 micro-semantic helper에만 허용한다.
- topology compiler는 owner와 모든 realized node에 `model_profile`을 hash-seal한다.
  route-bound dispatch는 이 profile과 portable role을 함께 adapter에 전달하며 trailing
  concrete model/effort로 조용히 바꿀 수 없다. 비-topology legacy/direct surface의 명시
  선택은 기존 자격 검증을 유지한다.
- Claude/Codex는 4개 profile을 실제 model+effort 조합으로 구분한다. OpenCode가 검증된
  effort/variant 축을 제공하지 못하면 `balanced-deep -> deep`으로 명시 축약하고
  `profile_granularity=collapsed`를 보고한다. 축약을 4단계 parity로 표시하지 않는다.

### 13.20.2 SD-89 — registry v5 parallel group과 원자 N-way batch

- registry `schema_version: 5`는 `replications`를 `parallel_groups`로 대체한다.
  group은 `{id,node,kind,min_intensity,width_by_intensity,join_policy,
  independence_axes,legs}`를 선언한다. kind 어휘는
  `replicate|explore|adversarial|verify`, join은 현재 `all`, width는 2~4다. 네 번째
  leg까지인 이유는 기본 governor cap 5에서 owner 1 + siblings 4를 원자 admit하기
  위해서다.
- `legs`는 순서가 고정된 `{suffix,perspective,model_profile}` 배열이다. 첫 leg는 원래
  node id를 유지하고 나머지는 `<node>-<suffix>`가 된다. compiler는 실제 intensity의
  width만큼 전개하고 모든 leg의 index/profile/perspective/group metadata, disjoint output,
  downstream depends_on/inputs를 결정론적으로 봉인한다.
- `cross-harness` independence는 모든 leg가 서로 다른 harness라는 뜻이 아니라 group
  전체가 지원 harness 둘 이상을 포함한다는 뜻이다. N이 harness 수보다 클 때 나머지
  leg는 다른 `model-profile` 또는 `perspective`로 상관된 실패를 줄인다. 요구 축과 실제
  달성 축을 batch receipt/manifest에 따로 기록하고, auto-selected cross-harness 불가 시
  typed degradation만 허용한다.
- autopilot-code framing은 standard에서 `balanced-deep anchor + light cross-harness
  alternative`, strong+에서 `deep contrarian`을 추가한다. strong plan은
  `deep anchor + balanced-deep alternative`, thorough+에서 `light implementation-risk
  scout`를 추가한다. strong implementation review는 `balanced-deep + light`,
  thorough+에서 `deep failure-mode`를 추가한다. 다른 capability의 기존 pair는 같은
  primitive의 width 2로 이관하되 자동 확장하지 않는다.
- `dispatch-batch`는 route가 선언한 exact N members를 한 manifest로 seal하고 governor
  lock 한 번에서 full-N slot을 원자 예약한다. 부족하면 row/model spawn 0이다. wrapper는
  leg별 opaque reservation을 claim하고 동시에 시작한다. 안정 attempt의 단일 missing-leg
  recovery는 나머지 N-1 member가 active 또는 completed임을 canonical peer-set proof로
  모두 증명할 때만 허용하며, 2-way singular peer proof는 schema-v1 read compatibility다.
- 모든 N-way child는 같은 depth-1 owner의 depth-2 sibling이다. worker가 fan-out하거나
  depth 3을 만들 수 없다. exact group join 뒤 owner가 합성하거나 후속 depth-2 reducer를
  시작한다. 다수결은 기본 merge가 아니며 artifact 주장·근거·실패 조건과 provenance를
  reducer가 비교한다.

**acceptance**: ① registry v5가 malformed width/profile/leg/axis/mini를 launch 전 거부
② standard/strong/thorough route가 각각 선언된 2/3/N leg와 disjoint output·downstream
wiring을 결정론적으로 seal ③ owner/node/leg profile override 불가, Claude/Codex
profile resolution과 OpenCode 축약 보고 동형 ④ full N capacity 부족 시 row/model 0,
충분 시 exact N concurrent wrapper starts ⑤ one-missing recovery는 exact N-1 peer-set
proof 없으면 spawn 0 ⑥ cross-harness+profile diversity와 typed degradation receipt
...
- **v23 정련**: final parent recheck + spawn + PID/start/PGID publish 원자성, PID-less exact row의 spawn-0 해석, PID reuse no-signal exact-row closure, Codex/Claude terminal source의 liveness/harvest 동형성은 결정론 fixture 대상이다.
- **v24 전환**: exact parent-attempt child selection, batch cardinality, non-model join interval, typed aggregate 크기/어휘, attempt-scoped delivered-set 원자 상태, undelivered sibling-registration/delivered harvest-only phase 전이, intermediate terminal suppression, final terminal 단일성, runtime probe/fallback 표시는 결정론 fixture 대상이다. stage 결과를 받아 다음 route 의미 결정을 내리는 것은 여전히 owner LLM 소유지만, 그 결과가 생길 때까지 기다리는 process scheduling은 supervisor 소유다.
