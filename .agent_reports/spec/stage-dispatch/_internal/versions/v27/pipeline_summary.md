...
## v20 update (2026-07-20) — quick registered-headless-only + dispatch surface terminology

- 승인 결정으로 SD-19를 supersede했다. 새 `effective_intensity=quick` route는
  registered headless capability-owner node 하나(`dispatch_depth=1`,
  `max_dispatch_depth=1`)만 허용한다. native-subagent, inline fallback,
  interactive/empty/unknown transport는 route emission 전 fail-closed한다.
- `direct` main-inline과 `standard+`의 checked same/cross-harness headless →
  native-subagent → inline fallback은 보존한다. quick retry도 registered-headless-only,
  one live attempt이며 exhaustion은 typed terminal failure다.
- SD-74가 portable logical topology를 `dispatch_depth` namespace로 한정하고 Codex
  `agents.max_depth`, runtime nesting, process ancestry와 분리했다. route node depth와
  attempt execution surface/registration evidence는 별도 필드다.
- SD-75가 wrapper transport, attempt execution surface, fallback hop의 closed
  vocabulary를 분리하고 Codex native subagent·Claude subagent·Claude agent-team
  teammate session·registered headless worker session을 명시 구별했다.
- 범위 제외: multi-capability composition/co-primary DAG, source implementation,
  concrete model/runtime config 변경. 구현은 별도 `autopilot-code` cycle 소관이다.
- assurance: standard/general 정책을 기록했다. registered depth-2 research worker와
  independent deep-review worker를 동기 polling/harvest했고 review artifact의 V20-R1~R6
  required changes를 본 atomic transaction에서 모두 반영했다. fast-reviewer 2회와
  별도 fast fact-checker는 이 transaction에서 추가 수행하지 않았으며 audit의
  official-source fact verification을 근거로 사용했다.
- guard note: owner sandbox에서 `.spec-grounding` read-marker write가 read-only로
  실패했으나 immutable route `rt-24a883963a582a4a`가 `spec_read=stage-dispatch-prd-v19`
  및 artifact guard 만족을 봉인했다. canonical write는 `spec-transaction.py`
  `prd-transaction` lock 아래 snapshot+3-file update로만 수행했다.
...
## v24 update (2026-07-23) — runtime-owned completion join

- 실제 Codex owner 세션의 5분 41초/26 후속 tool call을 재분석했다. parent-parking은
  비대기 작업 21건을 차단하지만 wait continuation 5건은 남기므로 근본 parity가 아니다.
- SD-14의 same-turn model polling을 supervisor 미지원 runtime의 checked fallback으로
  내리고, standard+ registered owner의 정상 경로를 exact parent-attempt batch join으로
  교체했다. LLM은 graph/gate를 결정하고 비모델 runtime이 wait/reap/delivery를 소유한다.
- Codex는 App Server same-thread turn lifecycle, Claude는 persisted session resume를
  adapter-specific realization으로 사용한다. 새 global broker/daemon/spool은 없고 기존
  direct launch, Fleet, completion marker, SD-77 orphan cascade는 유지한다.
- 구현 검토에서 초기 supervised guard가 첫 open child 이후 두 번째 sibling dispatch도
  막는 결함을 확인했다. v24를 attempt-scoped delivered-set 상태기로 정련했다:
  undelivered batch는 동일 parent exact dispatch만 추가 허용하고, receipt가 전달된 open
  batch는 exact harvest만 허용한다. Codex native hook과 Claude command-scoped PreToolUse
  hook이 같은 정책을 집행하며 user-owned runtime settings는 수정하지 않는다. 정상
  supervisor 종료와 abnormal owner watcher가 attempt-scoped 상태를 idempotent 정리한다.
- 구현·검증 cycle: `plans/2026-07-23_codex-dispatch-completion-join/`, route
  `rt-9160bbc17157a4ed`. dispatch 인프라 자기수정이라 source mutation은
  `STAGE_DISPATCH_INLINE_OK=1` 예외로 수행하고, 통합 diff의 registered Claude 독립
  review는 별도로 유지한다.
- 구현 closure: source main commit `9c0ecae6`. exact join/supervisor/phase
  guard/orphan/terminal/wrapper/conformance 집중 검증 90/90, parent capture 34건,
  portable guards 356/356, adaptation boundary와 diff check를 통과했다. Codex App
  Server same-thread 및 Claude same-session/command-scoped hook live smoke가 통과했고,
  registered Claude round-3 audit는 high/medium 0의 PASS 보고서를 남겼다. 감사
  worker의 부가 최종 문구는 strict terminal envelope에서 거부해 row만 명시 종료했으며
  terminal PASS로 둔갑시키지 않았다. 최종 registry는 open 0, orphaned conductor 0,
  supervisor state 0이다.

## v25 update (2026-07-24) — successor readiness + concurrent replica batch

- live strong cross-harness drill에서 execute의 semantic marker/terminal row가 outer
  governor보다 약 4.6초 먼저 닫혔다. owner와 draining lease가 남은 상태에서 replica를
  순차 호출해 첫 leg만 admission되고 둘째는 dispatch class cap 3으로 거부됐다.
- SD-79는 marker/Fleet stage done을 보존하면서 successor-ready에 exact outer-process
  quiescence를 추가한다. gate/join/wait/fallback은 한 classifier를 사용하며 cap 상향,
  marker 지연, arbitrary sleep을 금지한다.
- SD-80은 strong의 순차 plan DAG를 바꾸지 않고 review replica group만 한 batch로
  동시 실행한다. governor N-slot atomic reservation이 wrapper row claim보다 앞서므로
  check/acquire TOCTOU와 부분 admission을 닫고, transient namespace에서도 두 foreground
  wrapper를 병렬 유지한다.
- SD-81은 runtime process에 exact attempt id를 투영해 terminal/draining registry stage와
