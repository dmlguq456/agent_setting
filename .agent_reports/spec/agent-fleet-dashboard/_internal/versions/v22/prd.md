...
> · **v18 hotfix 2026-07-23** (wide를 `harness | session (branch) | context / stage | time` 4칼럼으로 통합하고 context와 NOW 사이 콜론 제거; 폭·stage/DAG 계약 불변)
> · **v19 hotfix 2026-07-23** (wide 헤더를 `harness | session (branch) | stages | time`으로 정정하고 칼럼 경계마다 1칸 확대·빈 stage `-` 표시; topology 계약 불변)
> · **v20 hotfix 2026-07-23** (dispatch wrapper PID와 실제 Claude runtime PID가 다른 `claude -p`를 attempt-owned stream session id로 결합해 context를 복구하고, 선형 route stage를 분사 owner 행에 한 번만 표시)
...
- **F-17 (라이브 제목 refresher — cross-harness fleet sidecar + no-tools 경량 LLM 워커, 사용자 승인 2026-07-10·공유 확장 2026-07-13)**: 하네스 원본 transcript에 쓰지 않고 **fleet 소유 neutral sidecar**로 진행형 재요약을 제공한다.
  ...
  - **(v21 표시 개정; v11 minor #1의 생성 계약 유지) 제목+부제 통합 — 한 호출 두 출력**: provider 호출 하나가 `TITLE:`/`NOW:` 두 줄을 반환하고, 제목은 3~6단어·40자, NOW는 대화 언어 1문장·저장 120자를 유지한다. 사이드카의 `summary` additive·15분 신선 창·부제 실패 시 제목만 저장하는 정직 강등은 불변이다. **표시 위치는 F-37이 대체**한다: interactive session은 wide/narrow/stack에서 정체성 카드 바로 아래 `context <gauge> …[   NOW]`를 사용한다. dispatch는 context block 없이 fresh NOW만 session 열에 표시하고, NOW 부재 시 subordinate line도 없다. **분사 세션 디바운스 150s**(메인 600s)는 유지한다.
...
- **F-22 (세션 name zone 반응형 예산, v16 ctx 이동 반영)**:
  - **wide**: 사용자 의미 칼럼은 정확히 `harness (model·effort) | session (branch) | stages | time` 네 개다. branch는 별도 칼럼이 아니라 session 제목 바로 뒤의 dim 괄호 suffix다. harness→session, session→stages, stages→right-flushed time의 각 경계는 v18보다 정확히 1 display-cell 넓히며, stage 증거가 없으면 stages 자리를 비우지 않고 ASCII `-`를 dim으로 표시한다. 세션 제목은 40 display-cols 고정 상한을 지키고 ctx는 F-37 subordinate line으로 이동한다. dispatch 이름의 F-15 24열 compact 상한도 유지한다.
  ...
  - **acceptance**: 60/100/120/168열의 wide/narrow/stack 주 행과 subordinate line 모두 터미널 경계를 넘지 않고, `session (branch)`·model/time 및 dispatch 정렬이 유지되며, ASCII/한글 제목·NOW는 display-cell 경계에서만 잘린다.
...
- **F-29 (native 서브 에이전트 호출 관측 — v8 minor #3, 사용자 확정 2026-07-15)**: 메인/분사 세션 외에 runtime-native 서브 에이전트(Claude Agent 도구, Codex `agents.max_threads`, OpenCode subagent)의 호출 현황을 표시한다.
  ...
  - **소스(하네스별)**: OpenCode = DB `session.parent_id`+`agent` 컬럼(2026-07-15 실측 확인 — 가장 완전, 토큰·비용 포함) / Claude interactive = 세션 transcript의 `isSidechain: true` 라인 + Agent tool_use↔tool_result 짝짓기, registered dispatch = canonical logs 경계·attempt id·Claude suffix를 검증한 attempt-owned stream-json에서 같은 lifecycle parser 재사용(최대 8 MiB bounded scan, mtime/size 캐시) / Codex = state DB thread spawn edge. 소스 부재에는 추측 표시 금지.
  ...
  - **표시 개정(v11, v22 연결 보정)**: 서브에이전트당 1행 스택+`⚡N` 배지 계약을 **소유 행당 가로 스트립 1행**으로 대체 — `⚡<type> <●|✓>  <경과> · <type> …` (활성=normal·상시 표시, 완료=dim·`a` 토글 시만; ⚡는 첫 라벨 밀착, 경과는 2칸 분리 상시 dim). 인셋 = 순수 공백(커넥터 없음): 세션 소유 6칸, 분사 소유 행은 depth당 +2(8/10). **분사 자식의 서브에이전트도** 그 dispatch 행 아래 같은 스트립으로 표시한다. 연결 우선순위는 attempt-owned `DispatchJob.subagents` → exact child-session association의 `Session.subagents`; wrapper PID 단독 조인은 authoritative source가 아니다. group/process와 wide/narrow/stack에서 같은 규칙을 사용한다. `⚡N` 배지는 폐기한다.
  ...
  - **불변식**: zero-injection(read-only) 유지, `--json` additive(`sessions[].subagents`와 `jobs[].subagents`), 소스 부재·파싱 실패 = 서브 행 생략(회귀 없음). dispatch subagent 연결은 context 수집·상속·추론을 재도입하지 않는다.
...
- **F-32 (분사 세션 요약·정체 1급화 — 제목 입양, 사용자 확정 2026-07-16 "지금은 메인 세션만 요약 에이전트가 붙는데 모든 분사 세션에 다 해당되게")**: 스케줄 절반은 F-17 v11 개정(분사 자식 포함) — 표시 절반이 본 항목. dispatch 행 이름 = **title→slug→key 체인**(세션 행의 title→name→slug와 동형): enrich가 이미 해석한 자식 세션의 fresh sidecar 제목을 그 자식을 대표하는 잡 행으로 입양한다. **v16부터 title만의 옛 `pid` 조인은 폐기하고 F-37b의 단일 association을 사용한다**: exact `(pid,proc_start)` → attempt-owned stream session id → exact binding 부재 시 단 하나의 `(harness,realpath(cwd))` 후보이며 PID reuse·중복 PID/session id·cross-harness·cwd 2+ 후보는 title/NOW 전체를 거부한다(F-26 — 오귀속이 부재보다 나쁘다). `DispatchJob.title` additive(None=기존 slug 정체성 그대로), 이름 존 예산·클립은 유지하고 stage는 F-36 공통 projection만 사용한다. dispatch context는 F-37a에 따라 표시·입양하지 않는다.
...
- **F-37a (interactive context + dispatch summary 분리)**: wide 주 행의 확장 context gauge, narrow/stack inline context telemetry, wide-only NOW 행을 이 계약이 대체한다. live interactive `Session`은 정체성 카드 직후·sub-agent strip 전에 wide/narrow/stack 공통 subordinate line 하나를 표시한다. context block을 먼저 읽고 fresh NOW와 **콜론 없이 세 칸 간격**으로 구분하는 `context <gauge> <NN%|—>[   <fresh NOW>]` 문법이며, `normal|tight|critical` band 문자열은 숨기고 퍼센트 숫자는 dim으로 낮춘다. interactive truth table:
  ...
  - live 둘 다 부재 → `context <empty-gauge> —`;
  - stale/dead → cached context/NOW와 무관하게 detail row 전체 생략.

  렌더러는 선두 context block을 먼저 예약하고 남은 오른쪽 폭에서 NOW만 display-cell/CJK 안전 tail-clip한다. **registered `DispatchJob`은 context window를 표시하지 않는다.** 분사 `claude -p` stream에는 해당 session이 소유한 `context_window_size` 실측이 없으므로 다른 session/model sample을 denominator로 빌려 퍼센트를 추론하지 않는다. dispatch에 fresh NOW가 있으면 session 시작 열에 summary-only line만 표시하고, NOW도 없으면 subordinate line 자체를 생략한다. `context` 문자열·게이지·퍼센트·`—` placeholder는 group/process 및 모든 폭에서 0개다.
- **F-37b (분사 자식 title/NOW association)**: title과 NOW를 각기 다른 조인으로 가져오지 않고 하나의 child-session association이 함께 공급한다. 결합 우선순위는 ① exact runtime `(pid,proc_start)`, ② 현재 registry attempt가 소유한 Claude stream-json의 단일 `session_id`와 child registry session id의 exact `(harness,session_id)` 일치, ③ 앞선 두 exact binding이 모두 없을 때만 단 하나의 `(harness,realpath(cwd))` child 후보 순이다. ②는 dispatch wrapper PID와 실제 `claude -p` runtime PID가 달라도 같은 attempt임을 증명하며, log path는 canonical dispatch logs 경계·attempt id·Claude suffix를 모두 검증한다. identity association parser는 session id만 읽고 usage/context denominator를 수집하지 않으며, F-29는 동일 소유 stream에서 Agent lifecycle만 독립적으로 읽는다. PID reuse, 중복 identity/session id, 서로 다른 stream session id, cwd 후보 2개 이상, cross-harness 매칭은 title/NOW 입양을 거부한다. 부모·자식 context 상속과 `DispatchJob.context` population은 금지한다.

- **F-38 (context pressure ⊥ intensity)**: interactive session의 context band는 `unknown|normal|tight|critical`(`tight>=70`, `critical>=85`)의 관측·표시 신호다. band 변화는 title/NOW 표시 외의 route/node state, intensity, stage graph, dispatch depth, model role/effort, QA/reviewer budget, test/verification, retry, completion gate, safety·permission guard, definition of done을 변경하지 않는다. missing/stale/malformed/source-sequence-regressed telemetry는 `unknown`으로 강등하고 정상 compaction 수치 하락은 허용한다. dispatch 행은 context pressure 표면 자체가 없다.
...
| 표면 | 필수 픽스처·언단 |
|---|---|
| wide/narrow/stack 및 overflow | `wide@168`, `wide@120`, `narrow@100`, `stack@60`에서 interactive identity → `context <gauge> …[   NOW]` → sub-agent 순서를 유지한다. dispatch는 fresh NOW가 있을 때 session 열 summary-only line만, NOW가 없으면 subordinate 0줄이며 두 경우 모두 `context`·게이지·퍼센트·`—`가 없다. 같은 route의 parent+quick owner fixture는 `one-shot`을 정확히 한 번만 표시하고 parent stage는 `-`이며 `quick/exec`·별도 linear stage detail이 없어야 한다. 모든 행은 CJK-safe clip과 폭 `<= terminal width`를 지킨다. |
| child 정체성·subagent | `(pid,proc_start)` exact 조인과 wrapper PID≠runtime PID인 attempt-owned stream `session_id` 조인이 title/NOW를 함께 복사한다. attempt-owned stream의 Agent lifecycle은 `jobs[].subagents`로 직접 연결하고, 부재할 때만 exact child `Session.subagents`를 사용한다. stream usage/context window를 읽거나 `DispatchJob.context`를 채우지 않는다. PID reuse·중복 PID/session id·서로 다른 stream sid·cwd 2후보·cross-harness는 title/NOW association을 보류한다. |
...
| JSON additive | 기존 `sessions`, `jobs`, `summary`, `route`와 하위 필드 의미를 유지하고 `work_projection`과 `jobs[].subagents`를 additive로 제공한다. interactive `sessions[].context`는 정규화하되 `jobs[].context`는 `null`이며 old-key-only consumer도 통과한다. |
| context 직교성 | interactive session의 69/70/85%에서 band/render만 변하고 route graph·node state·intensity·model/effort·QA·test·gate·guard는 byte-equivalent. 숫자 하락 자체는 허용하되 source-sequence regression은 unknown이다. dispatch context는 생성하지 않는다. |
...
flowchart TD
    ...
    WP --> J["--json additive<br/>work_projection · interactive context"]
    R --> TUI[fleet TUI]
...
- **F-37 lock (v21 correction, v22 subagent link)**: interactive session은 wide/narrow/stack의 primary identity row 아래에 콜론 없는 `context <gauge> …[   NOW]`를 표시한다. band 이름은 숨기고 퍼센트 숫자는 dim으로 낮추며 gauge threshold 색은 유지한다. dispatch는 context label/gauge/value/placeholder를 전부 생략하고 fresh NOW만 session 열에 표시한다. child title/NOW는 하나의 ambiguity-refusing association으로만 공급하고, subagent는 attempt-owned job evidence를 우선 연결한다.
- **F-38 lock**: interactive context pressure는 관측/표시만 바꾸며 intensity, graph/depth, model/effort, QA/test/retry/gate/guard/definition of done와 직교한다. dispatch context는 생성하지 않는다.
