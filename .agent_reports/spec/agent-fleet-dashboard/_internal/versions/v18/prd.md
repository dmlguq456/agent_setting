...
- **F-22 (세션 name zone 반응형 예산, v16 ctx 이동 반영)**:
  - **wide**: 사용자 의미 칼럼은 정확히 `harness (model·effort) | session (branch) | context / stage | time` 네 개다. branch는 별도 칼럼이 아니라 session 제목 바로 뒤의 dim 괄호 suffix이며, 기존 name+branch 총폭과 context/stage·time 정렬은 유지한다. 세션 제목은 40 display-cols 고정 상한을 지키고 ctx는 F-37 subordinate line으로 이동한다. dispatch 이름의 F-15 24열 compact 상한도 유지한다.
...
| 표면 | 필수 픽스처·언단 |
|---|---|
| wide/narrow/stack 및 overflow | `wide@168`, `wide@120`, `narrow@100`, `stack@60`에서 4칼럼 wide header, `session (branch)`, identity → `context <gauge> …[  NOW]` → sub-agent strip 순서, band 문자열·콜론 부재, 퍼센트 숫자 dim, gauge threshold 색 유지, CJK-safe clip, 행 폭 `<= terminal width`; NOW 부재, context 부재, 둘 다 부재, stale, dead 픽스처를 각각 고정한다. |
...
3. wide의 `harness | session (branch) | context / stage | time` 4칼럼과 narrow/stack의 동일 branch 괄호 문법, identity 아래 하나의 콜론 없는 `context <gauge> …[  NOW]` row를 구현한다. title/NOW/context 단일 child association, dim 퍼센트, 현행 TITLE 3~6단어·최대 40자 및 F-39의 3/4 concurrency·4 starts/60s·600/150s 계약은 유지한다.
