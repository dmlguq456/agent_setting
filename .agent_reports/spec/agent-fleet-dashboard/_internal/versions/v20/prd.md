...
- **F-17 (라이브 제목 refresher — cross-harness fleet sidecar + no-tools 경량 LLM 워커, 사용자 승인 2026-07-10·공유 확장 2026-07-13)**: 하네스 원본 transcript에 쓰지 않고 **fleet 소유 neutral sidecar**로 진행형 재요약을 제공한다.
  ...
  - **(v19 표시 개정; v11 minor #1의 생성 계약 유지) 제목+부제 통합 — 한 호출 두 출력**: provider 호출 하나가 `TITLE:`/`NOW:` 두 줄을 반환하고, 제목은 3~6단어·40자, NOW는 대화 언어 1문장·저장 120자를 유지한다. 사이드카의 `summary` additive·15분 신선 창·부제 실패 시 제목만 저장하는 정직 강등은 불변이다. **표시 위치는 F-37이 대체**한다: wide 전용 NOW 서브 행과 주 행 내 context gauge를 폐기하고, wide/narrow/stack 모두에서 정체성 카드 바로 아래 단 하나의 `context <gauge> …[   NOW]` subordinate line을 사용한다. **분사 세션 디바운스 150s**(메인 600s)는 유지한다.
...
- **F-37a (정확히 한 개의 context detail row)**: wide 주 행의 확장 context gauge, narrow/stack inline context telemetry, wide-only NOW 행을 이 계약이 대체한다. live session/dispatch 정체성 카드 직후·sub-agent strip 전에 depth inset을 맞춘 subordinate line 하나를 wide/narrow/stack 모두에 표시한다. context block을 먼저 읽고 fresh NOW와 **콜론 없이 세 칸 간격**으로 구분하는 `context <gauge> <NN%|—>[   <fresh NOW>]` 문법이다. v19의 harness→session 간격 확대 후에도 NOW가 session 시작 열에 좌측 정렬되기 위한 값이다. `normal|tight|critical` band는 데이터와 gauge 색상 판정에는 남지만 문자열로 표시하지 않으며, 퍼센트 숫자 자체는 threshold 색을 쓰지 않고 dim으로 낮춘다. truth table:
  ...
  렌더러는 선두 context block을 먼저 예약하고 남은 오른쪽 폭에서 NOW만 display-cell/CJK 안전 tail-clip한다. NOW 부재는 row 부재가 아니라 context-only로 정직하게 표시한다.
- **F-37b (분사 자식 통합 association)**: title, NOW, context를 각기 다른 조인으로 가져오지 않고 하나의 child-session association이 세 값을 함께 공급한다. 결합 우선순위는 ① exact runtime `(pid,proc_start)`, ② 현재 registry attempt가 소유한 Claude stream-json의 단일 `session_id`와 child registry session id의 exact `(harness,session_id)` 일치, ③ 앞선 두 exact binding이 모두 없을 때만 단 하나의 `(harness,realpath(cwd))` child 후보 순이다. ②는 dispatch wrapper PID와 실제 `claude -p` runtime PID가 달라도 같은 attempt임을 증명하며, log path는 canonical dispatch logs 경계·attempt id·Claude suffix를 모두 검증한다. stream의 최신 assistant usage에서 `input_tokens + cache_creation_input_tokens + cache_read_input_tokens`를 active context로 사용하고, fresh official statusline sample의 exact `model.id`가 제공한 `context_window_size`만 denominator로 허용한다. exact model window가 없으면 context는 `unknown`이다. PID reuse, 중복 identity/session id, 서로 다른 stream session id, cwd 후보 2개 이상, cross-harness 매칭은 세 값 전체를 거부한다. dispatch context는 해당 child runtime의 normalized telemetry만 사용하며 부모 context 상속은 금지한다. `DispatchJob`/JSON의 child `context{used_pct,band,source}`는 additive이고 구 필드 의미를 바꾸지 않는다.
...
| 표면 | 필수 픽스처·언단 |
|---|---|
| wide/narrow/stack 및 overflow | `wide@168`, `wide@120`, `narrow@100`, `stack@60`에서 `harness | session (branch) | stages | time`, 각 wide 칼럼 경계 +1 cell, 빈 stage `-`, identity → `context <gauge> …[   NOW]` → sub-agent strip 순서, band 문자열·콜론 부재, 퍼센트 숫자 dim, gauge threshold 색 유지, CJK-safe clip, 행 폭 `<= terminal width`; 같은 route의 parent+quick owner fixture는 `one-shot`을 정확히 한 번만 표시하고 parent stage는 `-`이며 `quick/exec`·별도 linear stage detail이 없어야 한다. NOW 부재, context 부재, 둘 다 부재, stale, dead 픽스처를 각각 고정한다. |
| child 정체성 | `(pid,proc_start)` exact 조인과 wrapper PID≠runtime PID인 attempt-owned stream `session_id` 조인이 각각 title/NOW/context를 함께 복사하고 부모/자식 서로 다른 ctx가 상속 금지를 증명한다. exact-model official window가 있을 때만 stream usage context%를 만들며 window 부재는 `unknown`이다. PID reuse·중복 PID/session id·서로 다른 stream sid·cwd 2후보·cross-harness는 세 값 전체를 보류한다. |
...
- **F-37 lock (v19 hotfix)**: wide/narrow/stack의 primary identity row에서 context를 제거하고 subordinate line을 콜론 없는 `context <gauge> …[   NOW]` 문법으로 통일한다. 세 칸 간격은 v19 session 시작 열에 NOW를 맞춘다. band 이름은 표시하지 않고 퍼센트 숫자는 dim으로 낮추되 gauge의 threshold 색은 유지한다. NOW 부재에는 context-only, context 부재에는 빈 gauge+`—`, dead/stale에는 row 전체 생략. child의 title/NOW/context는 하나의 ambiguity-refusing association으로만 공급하고 parent context를 상속하지 않는다.
...
3. wide의 `harness | session (branch) | stages | time` 4칼럼, 각 경계 +1 cell, 빈 stage `-`와 narrow/stack의 동일 branch·stage placeholder 문법을 구현한다. identity 아래에는 session 열에 맞춘 콜론 없는 `context <gauge> …[   NOW]` row를 둔다. title/NOW/context 단일 child association, dim 퍼센트, 현행 TITLE 3~6단어·최대 40자 및 F-39의 3/4 concurrency·4 starts/60s·600/150s 계약은 유지한다.
