...
| child 정체성 | `(pid,proc_start)` exact 조인과 wrapper PID≠runtime PID인 attempt-owned stream `session_id` 조인이 각각 title/NOW만 함께 복사한다. stream usage/context window를 읽거나 `DispatchJob.context`를 채우지 않는다. PID reuse·중복 PID/session id·서로 다른 stream sid·cwd 2후보·cross-harness는 title/NOW를 보류한다. |
...
| JSON additive | 기존 `sessions`, `jobs`, `summary`, `route`와 하위 필드 의미를 유지하고 `work_projection`을 additive로 제공한다. interactive `sessions[].context`는 정규화하되 `jobs[].context`는 `null`이며 old-key-only consumer도 통과한다. |
...
- **F-37 lock (v21 correction)**: interactive session은 wide/narrow/stack의 primary identity row 아래에 콜론 없는 `context <gauge> …[   NOW]`를 표시한다. band 이름은 숨기고 퍼센트 숫자는 dim으로 낮추며 gauge threshold 색은 유지한다. dispatch는 context label/gauge/value/placeholder를 전부 생략하고 fresh NOW만 session 열에 표시한다. child title/NOW는 하나의 ambiguity-refusing association으로만 공급한다.
