...
- **F-15 (분사 row 레이아웃 재설계 — 탈가로화 + 옵션 1급 유지, 사용자 피드백 2026-07-10 저녁)**: F-14 출하 후 사용자 최대 불만 = "분사 세션 명이 다양한 옵션과 함께 가로로 쭉 늘어짐". 단 **옵션(capability·mode·qa·intensity·model/effort)은 사용자가 관찰하는 중요한 요소 — 숨기지 말고 더 잘 설계하라**가 명시 요구.
  ...
  - **depth별 skill 정체성(v13, 2026-07-20)**: depth-1 options cell은 entry capability와 orchestration knobs를 표시한다(`code(dev/refactor·strong·owner)`). depth-2는 부모의 mode/entry를 반복하지 않고 **자신에게 배정된 stage skill**을 표시한다 — route가 있으면 `route_node`, 없으면 registry capability key (`research(strong)`, `code-test(strong)`). depth-2의 `worker_role=qa|development|deep maker` 같은 내부 persona와 상속 mode는 관제 정체성이 아니므로 숨긴다. QA level은 `--json` evidence에는 보존하되 사용자 화면에서는 intensity가 assurance를 대표한다.
...
- **F-17 (라이브 제목 refresher — cross-harness fleet sidecar + no-tools 경량 LLM 워커, 사용자 승인 2026-07-10·공유 확장 2026-07-13)**: 하네스 원본 transcript에 쓰지 않고 **fleet 소유 neutral sidecar**로 진행형 재요약을 제공한다.
  ...
  - **(v11 minor #1, 2026-07-19) 제목+부제 통합 — 한 호출 두 출력**: F-31의 문제의식("지금 무엇을 하는 중")을 별도 watcher 없이 이 refresher가 흡수한다(사용자 확정 "그때 제목이랑 내용을 함께 하면 되는거 아닌가? 한번에?"). provider 호출 하나가 `TITLE:`/`NOW:` 두 줄을 반환 — **제목은 3~6단어·40자로 압축**(부제가 디테일을 받으므로 "일종의 부제 느낌으로 요약을 넘기면" — 기존 4~8단어·64자 계약 대체, 이름 존 클립 해소 겸), **부제(NOW)는 대화 언어 1문장·저장 120자**. 사이드카에 `summary` additive(구 판독 호환), 부제 신선 창은 15분(제목 24h와 별도 — 실황은 빨리 낡는다). 부제 불량/부재 시 제목만 저장(정직 강등 — 부제 실패가 제목을 막지 않음). 표시 = 세션·분사 행 직후 dim 부제 서브 행 1줄(⚡스트립 위·같은 인셋 위계, dead/stale 생략[F-13], 없으면 무음, wide 전용). 분사 행은 F-32 입양 조인이 부제 동반. **분사 세션 디바운스 150s 단축**(메인 600s 유지, 폭주 상한 공유). 비용 = 호출 수·입력 동일, 출력 +~40토큰(사실상 0).
  - **(v11 개정) 분사 세션 전면 적용**: live fleet 스케줄러는 분사 자식 세션(`is_child`)도 메인 세션과 동일하게 요약 대상에 포함한다 — 사용자 2026-07-16 "모든 분사 세션에 다 해당되게" + "context 절감을 위해 haiku 에이전트가 하는 게 낫다"(부모 세션 context 비소모 구조 재확인). 내부 워커 재귀 차단은 `is_child`가 아니라 mem_worker 태그(`FLEET_TITLE_REFRESH`/`MEM_DISTILL`) 몫이므로 자기 요약 재귀는 구조적으로 불가. 폭주 상한(동시 2·창당 시작 16[2026-07-20 사용자 상향, 구 10]·600s 디바운스)은 자식 포함 전체가 공유. 관측 특성: 스케줄러는 live TUI 루프에서만 돈다 — 보드가 떠 있는 동안만 제목이 생성된다(관측이 곧 구동).
...
- **F-22 (세션명이 터미널 가로폭을 채우는 반응형 name zone, 2026-07-14 사용자 요구)**:
  - **wide**: 터미널 폭에서 branch·model·context·time과 패널 inset을 먼저 예약하고 남는 slack을 세션 name column에 준다. 세션 제목은 이 동적 예산까지 표시하되 child-count 공간을 먼저 보존한다(게이트 태그 공간 예약은 v11 minor #2로 소멸). dispatch 이름은 F-15의 24열 compact 상한을 유지하고 늘어난 name column에는 padding만 추가해 공통 컬럼 정렬을 보존한다.
