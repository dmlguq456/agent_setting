...
- **guard↔write_scope 정합성 (v10 SD-46)**: `spec/**` write를 선언한 node는 그 capability가 spec의 sole-update-path 소유자(`autopilot-spec`)이거나 conductor 선보장 gate(SD-13)를 선언해야 함 — registry 허용 write를 artifact-guard가 막는 조합을 validation 시점에 fail-closed로 거부. runtime에서 guard가 route-승인 write를 차단하면 조용한 실패가 아니라 structured failure + route record 참조를 냄
...
- completion gates
- **tracking 축 값과 escalation 근거 신호 (v10 SD-44)**: tracking은 intensity/topology/worker_kind/transport와 독립인 다섯째 필드. tracked는 산출물 계약만 결정하며 tracked 사실 단독은 escalation 근거 필드에 올 수 없음. quick 경계는 현행 유지(애매하면 quick depth-1 — 사용자 (a), 2026-07-15)이고 direct/quick/standard+ 선택 근거 신호를 기록
- **tracked gate 통과 증거 4종 (v10 SD-45)**: spec-read 여부, drift verdict, tracked/untracked mode, artifact-guard 전제 충족(SD-13 선보장 결과). worker는 route record hash를 검증하고 배정 node scope만 실행하며 capability/intensity/topology를 재선택하지 않는다 — bootstrap의 `status/prompt-signal/mode/route` 재실행은 record 검증+안전 확인(absolute cwd·canonical root·git state) 전용으로 축소(3어댑터 동형). 증거 4종이 없으면 tracked 대상 worker는 시작하지 않음
...
- guard↔write_scope 정합성 negative fixture reject — `spec/**` write 선언 + 비소유 + 선보장 gate 부재 (v10 SD-46)
- route record에 tracking 필드 존재 + escalation 근거 필드에 tracked 사실 단독 불가; tracked gate 증거 4종 부재 시 tracked worker 시작 거부 (v10 SD-44·45)
- worker가 record hash 불일치·배정 node scope 밖 실행을 거부하고 capability/intensity 재선택을 하지 않음 — 3어댑터 fixture 독립 검증 (v10 SD-45)
...
- 두 세션 동시 spec-transaction fixture: 후행 세션이 §5.8 lock BLOCKED 후 대기 → 최신 재독 → 다음 버전 번호 진입, 동일 `v{N}` snapshot 이중 생성 0건 (v10 SD-47)
- `versions/v{N}` 체인 갱신이 lock 보유 중에만 발생; spec-touch 미선언 route의 spec write는 structured failure (v10 SD-47)
