...
## 이연 항목 처리 (같은 날 2차 사이클)

- **qa 리뷰 템플릿·return 계약 승격 — 실행**: `agent-modes/qa/_review_rules.md` 신설(design `_design_rules.md` 기존 패턴 재사용, 신규 장치 아님). 심각도 골격(🔴🟡🟢)과 1줄 반환 계약을 단일 원천화, code-review·plan-review·test 모드는 고유 정의(헤더·항목 식별자·필드·verdict 토큰)만 보유. qa-team 라우터에 include 선독 의무 추가. 정적 바이트 +448B이나 3중 이중 정의 드리프트 원천 제거.
- **dev 라우터 중복 정리 — 실행**: backend 금지구역 블록·refactor Core Rules 4개조가 라우터 Common Rules를 재기술하던 것을 포인터+모드 특화분만으로 축약.
- **dev confirm 규칙 승격 — 기각(필요 중복)**: 4개 모드의 confirm 규칙은 범위·형태가 모두 다름(backend=신규 코드만, frontend=신규 컴포넌트, new-lib=call-site 우선 설계, refactor=interactive 전체). 승격 시 모드별 의미가 평탄화되므로 유지.
- **라우터 간 공통 블록(R5) — 기각(필요 중복)**: plan/dev/qa/editorial 라우터는 서로 독립 런타임 표면(서브에이전트 시스템 프롬프트)이라 컨텍스트를 공유하지 않음. agents/ 디렉터리의 .md는 전부 에이전트로 등록되므로 공유 include 파일을 둘 수 없고, 신설은 §0.5 신규 장치 요건 위반. 각 라우터의 언어 규칙·mem profile·spec-backed 블록은 표면별 필수 사본으로 판정.
- **code-report 페르소나 갭(4.1) — 기각**: qa-team에 report/writer 모드를 신설하는 것은 새 표면 추가라 최소주의 원칙과 충돌. 리포트 페르소나의 스킬 인라인 소유는 의도된 설계로 유지.
