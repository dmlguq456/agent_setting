...
- `bash tools/memory/mem_retrieval_v14.test.sh` — PASS=20, FAIL=0.
  The new regression proves a fixed natural-language phrase cannot change
  automatic mode, threshold, or result IDs; Korean particle/CJK recall remains
  intact.
...
- `tools/check-adaptation-boundary.sh` — pass. The existing informational warning
  about 56 documented concrete runtime/model references remains.
