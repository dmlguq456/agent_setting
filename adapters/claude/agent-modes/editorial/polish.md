...
**언제 호출되는가**: 산출물의 _언어 자체_ 는 맞는데 _표기 일관성·불필요한 언어 혼용·번역체·가독성_ 에서 어색할 때. 어느 언어의 산출물이든 해당 언어와 청중에게 자연스럽게 다듬는 것이 본 모드다.
...
1. 문서를 끝까지 읽는다.
2. 문장 단위 점검:
   - 청중의 이해를 해치는 차용어·음역어는 대상 언어의 자연스러운 표현으로 재서술
   - 설명되지 않은 다른 언어의 용어가 흐름을 깨면 청중에 맞게 분할하거나 풀이
   - 원문 구조를 옮긴 직역체·부자연스러운 어순은 대상 언어의 자연스러운 어순으로
...
5. 변경 요약 (어떤 표현을 어떻게 바꿨는지) 을 사용자의 현재 소통 언어로 3-5 줄 보고. 명시된 보고 언어·외부 청중 계약이 있으면 그것을 우선한다.

## Catch-net — writing-craft 위반 신호 (refine 권장 항목)

본 모드는 _다듬기_ 단계에서 _이미 만들어진 산출물_ 을 손본다. _author 시점_ 의 단락 cohesion / 자연 통합 / tone 결정은 **strategy·draft 작성 단계** 책임 (`skills/draft-strategy/SKILL.md` _Paragraph Cohesion Pre-Check_ + _Natural-integration rule for paper-body mutations_ + _Tone Auto-Detection_ 절). 본 모드는 그 결과물을 _catch-net_ 으로 점검만 한다.
