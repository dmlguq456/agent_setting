...
mkdir -p "$LOCAL_BIN"
cat > "$LOCAL_BIN/fleet" <<EOF
#!/usr/bin/env bash
# fleet launcher (Windows/Git Bash) — generated by install-windows.sh.
exec bash "$CLAUDE_DIR/tools/fleet/fleet.sh" "\$@"
EOF
chmod +x "$LOCAL_BIN/fleet"
log "installed fleet launcher: $LOCAL_BIN/fleet"
case ":$PATH:" in
  *":$LOCAL_BIN:"*) ;;
