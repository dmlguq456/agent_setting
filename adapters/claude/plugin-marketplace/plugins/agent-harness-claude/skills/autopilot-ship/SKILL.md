...
Prepare application deployment and release setup. Accumulate the current shipping contract in `<artifact-root>/spec/ship.md` under the [three-tier output convention](../../core/CONVENTIONS.md#5-skill-output-convention--t1t2t3).
...
- Derive verification rigor from `--intensity`; there is no separate `--qa` axis. Use the [canonical mapping](../../core/CONVENTIONS.md#11-verification-rigor-tiers).
...
Read [references/examples.md](references/examples.md) for initial-setup, environment-update, and domain-connection examples.
