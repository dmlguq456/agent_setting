...
| Step 6·6b·7 + Hook | Sync state 갱신 → manifest.json 재방출 → Final report + Hook integration(옵션) | `references/finalize-and-hooks.md` |

## Required Reads

- drift 감지 실행 (Step 1~3 — discover/hash, sync state v4 스키마, drift report 형식): `references/detect-and-report.md`.
- README 재생성 (Step 4·5·5a — 4a 트랙 체인, 4b 9섹션 canonical layout·_넣지 않음_ drop+link, 섹션별 자동 갱신 정책 표, 편집팀 검수): `references/readme-dashboard.md`.
- Cross-doc drift 스캔 (Step 5b·5c·5d — QA/model role invariant 표, skill name rename scan, 산출물 폴더 컨벤션, 매뉴얼 동기 검토): `references/cross-doc-invariants.md`.
- 종료 (Step 6·6b·7 — sync state 저장, manifest.json 재방출, final report, Hook integration): `references/finalize-and-hooks.md`.

## Reference Map

- `references/detect-and-report.md`: Step 1(Discover + hash), Step 2(Read sync state — v4 스키마), Step 3(Drift report — 4 분류·한국어 출력·`--check` 종료).
- `references/readme-dashboard.md`: Step 4(Generate dashboard — 4a 트랙 텍스트 체인, 4b README 9섹션 canonical layout·원칙·_넣지 않음_ drop+link), Step 5(Write README.md — 섹션별 자동 갱신 정책 표), Step 5a(편집팀 검수).
- `references/cross-doc-invariants.md`: Step 5b(QA/model role invariant scan 5b-1~5b-4), Step 5c(skill name rename drift 5c-1~5c-5), Step 5d(에이전트 엔지니어링 매뉴얼 동기 검토).
- `references/finalize-and-hooks.md`: Step 6(Update sync state), Step 6b(Emit manifest.json), Step 7(Final report), Hook integration(옵션 Stop hook).
