...
### Step 6c: skill-conformance 결정론 gate (§CONVENTIONS 5.6a)
...
```bash
bash tools/skill-conformance/check.sh skills adapters/claude/skills
bash tools/skill-conformance/scan.sh skills   # 필요 시 raw TSV 관측
```

- `--check` 모드에서 `check.sh`를 실행한다. `line_ok=N`, `ref_depth_ok=N`, invocation registry 위반 중 하나라도 있으면 비-0으로 실패시키고 Step 3 drift report / Step 7 final report에 노출한다. 신규·수정 스킬은 통과가 merge 전제.
- invocation registry(`tools/skill-conformance/invocation-policy.tsv`)는 parent/pipeline 호출 13개를 model-invoked(`disable_model=false`)로 고정한다. `disable-model-invocation: true`는 registry에 `user-only`로 명시된 manual workflow만 허용한다. entry-router를 registry에 분류하면 model-invoked + "Use when"도 함께 검사한다.
- 회귀 게이트는 drill `g7_skill_conformance` (`loops/drill/cases/`)가 양 Claude skill 트리와 parent/user-only failure control을 static-assert로 검증한다.
