...
- **Claude Skills**: `<agent-home>/adapters/claude/skills/*/SKILL.md` (frontmatter)
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "find <agent-home>/adapters/claude/skills <agent-home>/adapters/claude/agents -name '*.md' -newer <agent-home>/skills/.sync_state.json 2>/dev/null | head -1 | grep -q . && echo '[sync-skills] drift detected — run /sync-skills' || true"
      }]
    }]
  }
}
...
---
*Claude adapter realization: `<agent-home>/adapters/claude/skills/sync-skills/SKILL.md`; compatibility reference: `<agent-home>/skills/sync-skills/SKILL.md`*
