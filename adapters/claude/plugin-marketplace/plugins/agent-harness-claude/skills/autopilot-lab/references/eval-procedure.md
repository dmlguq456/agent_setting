...
| Follow-up note cycle | `autopilot-spec` update when applicable, then the DB-gated `autopilot-note` handoff | Finalized experiment results | Updated note row or typed skip | After terminal E3-4: connected requires `/autopilot-note --from <experiment-path>`; unavailable records `skipped/db-unavailable` |
...
- After `REPORT.md`, `STORY.md`, `_RUNLOG.md`, and `run.json` are durable, apply the route-sealed DB readiness condition for done and failed terminal states with durable artifacts. On `connected`, invoke the required `follow-up note cycle` with `/autopilot-note --from <experiment-path>`; on unavailable, record `skipped/db-unavailable`. User suggestion does not override either branch.
