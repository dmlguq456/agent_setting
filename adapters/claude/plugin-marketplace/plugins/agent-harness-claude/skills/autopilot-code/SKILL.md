...
|---|---|---|
| `references/context-and-guards.md` | Every invocation (required) | Artifact, spec, and git guards; spec-mode detection; design/app/library/API/CLI/research boundaries; experiment-ready input; invocation routing |
| `references/arguments-and-decisions.md` | When interpreting arguments, `--from`, pause/resume, or active-plan conflicts | Argument parsing, defaults, active/partial/complete plan handling, and plan-path resolution |
| `references/dev-pipeline.md` | When running `--mode dev` | Stage orchestration, plan check, retry behavior, and `analyze-project` update |
| `references/debug-audit.md` | When running `--mode debug` or `audit` | Debug diagnosis and fix flow; audit fan-out and autofix workflow |
| `references/pipeline-summary-safety.md` | At terminal, failed, partial, rollback, or summary states | Summary template, terminal-state reporting, and common safety rules |
...
| `standard` | code-plan → plan-check → bounded depth-2 verifier/planner when separable → synthesize → code-execute → code-test → code-report | Required | Use bounded depth-2 planning or verification for separable work |
| `strong` | standard + one risk-focused depth-2 review | Required | Review the single riskiest point |
| `thorough`/`adversarial` | standard + multi-axis depth-2 planner/verifier/adversary synthesis | Required | Synthesize depth-2 reports concisely |

**`standard+` dispatch**: Run each durable stage (`code-plan`, `code-execute`, `code-test`, and `code-report`) in its own depth-2 headless session. The depth-1 conductor passes artifact paths, reads only verdict and status, and uses deterministic one-shot `dispatch-wait` polling for wait and harvest (dev-pipeline Steps 1-7; OPERATIONS §5.10 and SD-14). Keep `direct`, `quick`, and plan-check micro-stages inline.
