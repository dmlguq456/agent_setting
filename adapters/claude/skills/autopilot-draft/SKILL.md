...
- `references/invocation-and-args.md`: Argument Parsing 전문(mode/auto-inference/input discovery/intensity-derived rigor/`--user-refine`/`--from`/format spec resolution), Decision Defaults 표, `pipeline_state.yaml` 스키마, Input Sources Convention, Artifact Structure.
...
- `references/review-and-qa.md`: Step 3 Strategy Review + Step 5 Draft Review — intensity-derived rigor 별 reviewer 수·axis 분해(A domain/content · B methodology/writing · C style · D cross-ref+coverage)·quality reviewer 프롬프트·fact-checker 프롬프트.
