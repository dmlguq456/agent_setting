...
description: "Use when invoking the portable autopilot-apply capability. cheatsheet 초안을 실제 source artifact에 적용하고 검증한다."
...
metadata:
  ...
  fam: doc
  modes: []
  blurb: "cheatsheet 초안을 실제 source artifact에 적용하고 검증한다."
---
...
2. **git working-state** ([OPERATIONS.md §5.9](../../core/OPERATIONS.md#59-git-working-state-preflight-worktreemerge-가드-canonical)): source 파일에 커밋 안 된 사용자 편집(dirty)이 있거나, **merge/rebase/cherry-pick 진행 중·detached HEAD** 면 abort + "먼저 commit/stash 하거나 진행 중 머지를 해결하세요." (진행 중 작업·반쯤 머지된 트리 클로버 방지)
