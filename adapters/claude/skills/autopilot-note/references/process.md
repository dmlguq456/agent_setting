...
- _Catalog emergence_: overly broad or overly conservative decisions
...
5. **Maintain manifests on every run (PRD v33):** scan the backbone catalog and:
   ...
   - For a backbone whose body already has content, especially `manifest_status: confirmed`, do not edit directly. Stage a review note when new derivations or changed uses should be recorded.
...
The digest is a review group for each nightly run, not merely a count summary. Put the `run_id` header and low-confidence or ambient items first so the board home provides an effective correction entry point.
...
- This run: created <N> · **review needed (low confidence or ambient) <I>** · catalog entries emerged <E>
- Notes: <N> (linked <P> / ambient <A>)
- Catalog emergence: backbone <B> / task <T> / paper <Pa>

### ⚠ Review needed (low confidence or ambient — correct on the board note screen)
...
• Notes: <N> (routing_status: confirmed)
...
Next: correct low-confidence or ambient card attribution on the board note screen; all routing is agent-confirmed.
```
