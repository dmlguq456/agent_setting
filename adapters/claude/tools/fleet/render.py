...
def _wide_name_width(term_width):
    ...
    Reserve every field after the name plus the right-flushed time and panel margins, then
    give the remaining horizontal slack to the session column — but only up to
    _NAME_WIDE_MAX (F-22 minor, v8). Leftover slack past the cap is deliberately left as
    end-of-row padding rather than being handed to another column: the point is a stable,
    predictable name zone, not a wider one.
    """
    if not term_width:
        return _NW_S
