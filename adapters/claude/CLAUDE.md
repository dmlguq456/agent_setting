...
- **High-stakes → intensity 상향**: _신중히 / 꼼꼼히 / camera-ready / submission·PR open 직전_ → intensity 를 strong/thorough/adversarial 로 자동 상향 (검증 rigor 는 intensity 에서 파생 — CONVENTIONS §1.1). `analyze-user` 는 항상 intensity=adversarial 고정.
- **무응답**: §2 대로 추천안 자율 진행.

**(C) 작업 격리·병렬 디스패치 (`OPERATIONS.md` §5.10).** 코드 본작업은 worktree+작업 브랜치에서 — **기능 추가·모듈 신설·다파일 변경은 규모 판단 없이 무조건 브랜치, 애매해도 브랜치 쪽** (drill g3 재발 방지). main 트리 직접은 typo·1줄급 자잘한 단발만. 작업 진행 중 새 독립 요청 → 파일 겹침 triage 후 새 worktree 로 background 병렬 분사 (겹치면 큐잉). 오케스트레이션은 항상 main (in-session nested subagent 는 hook 발화·skill·worktree·jobs.log·statusline 풀 ceremony 격리를 못 받음 — **`standard+` 다파일·기능 본작업은 worktree 안 `claude -p` headless 분사가 의무 (재량 아님; direct 은 inline, quick 은 depth-1 one-shot worker, 마이크로-스테이지만 inline — drill g6)**, 이때 main 이 작업 난이도별 `--model-role` 또는 `--model/--effort` 를 명시 선택. main 은 depth-1 conductor 를 분사하고, 그 **conductor(depth-1)는 `standard+` 파이프의 각 스테이지(code-plan·execute·test·report)를 depth-2 headless 세션으로 분사**한다 — 세션 간 소통은 산출물 파일만, conductor 는 verdict/status 만 쥔다 (2026-07-10 stage-dispatch, OPERATIONS §5.10 ③④). in-session 격리 한계가 오히려 스테이지 분사를 정당화. direct/quick·마이크로-스테이지는 inline, depth 3+ 금지) (스테이지 분사 기본의 예외 = **separability 판정** — 비분리 편집만 inline 허용 + metrics 기록 의무, OPERATIONS §5.10 ③ SD-17), merge 는 Claude 선별 책임이되 _사용자 머지 신호·병렬 job 수확 자리 한정_ — 본작업은 브랜치에 남기고 main 불변으로 turn 종료, 같은 turn self-merge·브랜치 삭제 금지 (OPERATIONS §5.10 — diff 실내용 확인·회귀/중복 제외·충돌 양쪽 의도 해석·애매하면 질문·빌드 검증). **편집 전 git 상태**: merge/rebase 진행 중·detached HEAD = STOP+보고 — 직접 편집 경로 포함, `git-state-guard` hook 강제 (OPERATIONS §5.9). **머지 완료된(ahead 0) 죽은 브랜치 위에선 직접 편집도 금지** — 새 브랜치 먼저 (OPERATIONS §5.9 DONE-BRANCH). **독립 분사 즉시화 (대기 금지)**: 사용자 지시 작업 중 _다른 미결정(파라미터 답변 대기 등)에 의존하지 않는_ 독립 부분은 그 미결정을 기다리지 말고 _즉시_ 병렬 분사한다 — 전체를 한 미결정에 묶어 세우지 않기. 단 분사도 정규 경로(코드 = `autopilot-code` · worktree) 경유, 형식·설계 결정은 커밋 전 노출 (빠른 분사 ≠ 컨벤션·리뷰 우회). **분사 후 stealth-death 가드 (필수)**: hung/crash 한 headless 는 완료 알림이 안 와 무한 대기 위험 → 완료 알림만 믿지 말고 대기 자리에서 `utilities/dispatch-liveness.sh` 로 transcript-mtime liveness 점검 (SUSPECT/DEAD 면 진단→수확/재분사, 대기 X). **conductor 는 one-shot 프로세스이므로 스테이지 분사 후 알림 대기로 turn 을 끝내지 말고 `utilities/dispatch-wait.sh --parent <conductor-slug>` 로 같은 turn 폴링·수확한다 (SD-14; Claude `-p` 는 Stop hook 미발화라 대기 강제는 dispatch-wait 로만). 상세는 core/OPERATIONS §5.10 one-shot 대기 계약이 SoT — 본 절은 포인터.** 상세 OPERATIONS §5.10.
...
- **입력 이상 ≠ 산출물 보류**: 입력 데이터가 손상·placeholder·부분 상태여도 요청 산출물 생성이 가역적이면 _만들고_ 그 상태를 산출물·응답에 명시한다 (추천안 = 구조 생성 + 상태 표기 + 결과 뒤에 질문). 가역 작업에서 산출물 0 + 선택지 질문으로 turn 종료 금지 (roles/response-policy "Degraded input" 실현).
...
- **Memory semantic boundary (D-40)**: the acting agent—main, distiller, or curator—decides contextually what to store, retrieve, promote, merge, or prune. Scripts enforce DB integrity, scope isolation, pending protection, action validation, bounded lifecycle mechanics, and recovery. Prompt hooks do not classify every user message for recall. `[pending:<id>]` is consumed only with `mem consume <id>` after the obligation has been applied and verified; retrieval alone is non-consuming. Details: MEMORY §7.
...
- **제품 진입점·워크플로우 안내**: `~/.claude/README.md` (human-owned)
...
| **Agent-chosen memory retrieval** — when the agent judges that prior context may materially improve the current decision. There is no fixed phrase, mandatory topic list, or prompt classifier. | Inline: `~/.claude/tools/memory/recall.sh "<query>"` / deep search: `Agent(memory-scout)` | The agent chooses the query and search depth. Use the read-only scout for raw sessions, multiple query angles, or cross-project retrieval. Cross-check results against live code. If retrieval exposes `[pending:<id>]`, read the full obligation, apply and verify it, then run `mem consume <id>`. Details: MEMORY §7.4. |
...
산출물 폴더 구조 = `core/CONVENTIONS.md` §5 (single source, 재정의 금지) · 트랙별 폴더 맵 = `core/WORKFLOW.md` · skill scope 경계 = 각 SKILL.md·`capabilities/` 정의 · 공통 플래그 (`--intensity`/`--from`/`--user-refine`) 정의 = CONVENTIONS §1 (검증 rigor 는 `--intensity` 파생 — 별도 `--qa` 축 없음). 산출물 중복·정의 외 추가 생성 시 멈추고 확인.

### 자주 빠지는 함정
- pipeline 이 sub-skill 호출 중인데 또 부르기 → 덮어쓰기
- artifact_dir 오타 (research vs documents vs plans)
- PPTX 자동 생성 (presentation 은 markdown 만)
- 처음부터 `--intensity thorough`/`adversarial` — 작은 요청은 `--intensity quick`, 본작업은 standard 부터 상향
