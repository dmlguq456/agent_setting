...
- **사용자 메모리**: `~/agent_setting/memory/memory.db`(SQLite 진실원천, 전용 repo `claude-memory` 가 그 자리에 nested — `dump.jsonl` 텍스트 mirror 추적, config repo 에선 `memory/` gitignore; 구 `~/.claude/memory` 는 2026-07-03 병합 이관 후 `memory.archived-2026-07-03` 로 보존) · 세션 주입 source · write 경로 = `mem`(DB) 단일 (`~/.claude/projects/<cwd>/memory/` 내장 file 메모리는 _직접 사용 X_ — `builtin-memory-guard` hook 차단; SessionEnd `mem sync` 는 stray 내장 write 만 DB 안전망 흡수)
- **사용자 프로필**: DB `memory.db` `type=profile` 레코드 (`mem profile <aspect>` 로 읽음; 구 `user_profile/0*.md`·`README` 는 DB·`MEMORY.md §7.6`(aspect↔agent 매트릭스) 으로 이전 — `assets`·`_internal` 만 로컬 잔존(analyze-user 자료, gitignore), `mem export --target profile` 은 on-demand 사람 열람 캐시) — cross-project 사용자 산출물 패턴 (figure / writing / presentation / analysis / domain / coding)
...
| **과거 결정·교정·컨벤션 회상 필요** (이 프로젝트의 _왜 이렇게 정했나·전에 어떻게 합의했나_ 류인데 세션 시작 주입 메모리·인덱스로 안 풀릴 때) **+ 회상 신호어 (B1 — D-15 hook 자동주입)**: _지난번·예전에·전에·그때·저번에·아까_ 류 신호어는 `mem-recall-inject.sh` hook 이 답하기 _전_ `mem recall` 결과를 `additionalContext` 로 자동 사전주입 (메인의 "recall 할까" 판단 0, §0.5; line 68 과 동일 메커니즘). recall.sh 수동 경로는 그 외·심층 회상(`--sessions`/`--all`)용 | `~/.claude/tools/memory/recall.sh "<query>"` (읽기 전용; 현 cwd 기본 / `--all`=cross-cwd / `--sessions`=raw 대화까지) | **Claude 가 직접 실행 — 사용자가 쉘 칠 일 없음.** recall.sh = `mem recall` thin wrapper(store FTS5 색인 + LIKE/rg fallback). 정제 메모리 먼저, 안 나오면 `--sessions`. 신호어 자리는 매 턴, 그 외는 필요 자리만(token 절약). 결과는 live 코드로 교차검증(메모리=작성시점 진실, stale 가능). `index-check.sh` = *legacy `projects/<cwd>/memory/` MEMORY.md 텍스트 인덱스 전용*(store 색인은 `mem index`). 상세 = MEMORY §7.4. |
...
| Skill | Artifact |
|---|---|
| `analyze-project --mode code/paper/doc` | `<artifact-root>/analysis_project/{code,paper,doc}/` |
| `autopilot-research` | `<artifact-root>/research/{topic}/` |
| `autopilot-draft` | `<artifact-root>/documents/{date}_{name}/` |
| `autopilot-spec` | `<artifact-root>/spec/` (청사진 — prd.md·stack.md·design/·ship.md, 항상 최신 T1) |
| `autopilot-code` | `<artifact-root>/plans/{date}_{slug}/` (작업 사이클) |
| `autopilot-lab` | `<artifact-root>/experiments/{date}_{slug}/` (+ `_RUNLOG.md`) |
