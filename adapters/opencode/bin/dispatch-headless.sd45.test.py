...
class OpenCodeSD45(unittest.TestCase):
 def test_route_consumer_and_capability_reselection_refusal(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   bad=dry.copy(); bad[bad.index("autopilot-code")]="audit"; denied=subprocess.run(bad,text=True,capture_output=True,env=env); self.assertEqual(denied.returncode,65); self.assertIn("capability-reselection",denied.stderr)
