#!/usr/bin/env sh
# OpenCode adapter-owned material data-script launcher.
set -eu
...
Checks or runs a Python data-analysis script through an OpenCode-owned material
...
if [ "$#" -eq 0 ]; then
  ...
  printf 'tool_contract_check=adapters/opencode/bin/preflight.sh data-script --check <script.py>\n'
...
[ "$#" -ge 1 ] || { echo "opencode data-script: script path required" >&2; exit 64; }
script=$1
shift
if [ "${1:-}" = "--" ]; then
  shift
