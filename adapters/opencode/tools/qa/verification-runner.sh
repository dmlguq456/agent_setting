#!/usr/bin/env sh
# OpenCode adapter-owned verification command runner.
set -u
...
Runs an explicit read-only verification command through an OpenCode-owned
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --check)
      ...
      ;;
    --timeout)
      [ "$#" -ge 2 ] || { echo "opencode verification-runner: --timeout requires seconds" >&2; exit 64; }
      timeout_s=$2
      shift 2
      ;;
    --)
...
if [ "$#" -eq 0 ]; then
  if [ "$check_only" -eq 1 ]; then
    ...
    printf 'tool_contract_check=adapters/opencode/bin/preflight.sh verification-runner --check -- <command>\n'
