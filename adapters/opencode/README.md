...
| memory candidate exposure and deeper retrieval | OpenCode captures the current user prompt transiently, runs `preflight.sh candidates` during the system transform, and immediately discards the prompt. The result is capsule-only, active current-project/global, and bounded to three headlines/IDs and 1,200 UTF-8 bytes. The same-turn message ID flows into the mutation gate. The model decides relevance and reads full records; explicit `recall` remains available for deeper search and `recall-gate` for hook recovery |
