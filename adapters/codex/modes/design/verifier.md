...
1. **preview** — `adapters/codex/bin/preflight.sh visual-harness <file.html>` 로 자기 브라우저에 로드.
...
| id | 설명 | visual-harness 경로 |
|---|---|---|
| `console.errors_zero` | 콘솔 에러·pageerror·네트워크 실패 0건 | `adapters/codex/bin/preflight.sh visual-harness <file.html>` 또는 adapter equivalent (Bash, headless Chromium via playwright — postwrite hook 동일 스크립트; exit 2 = 에러 있음). rendered evidence로 동등한 콘솔 조회 경로 |
...
> `color.contrast_pass` (본문 ≥4.5:1) 및 `typography.scale_pass` (슬라이드 본문 ≥24px / 모바일 hit-target ≥44px) 는 렌더 런타임이 계산된 스타일(대비·크기)을 보고할 수 있을 때 결정론 판정; 보고 못 하면 `unavailable` 강등 (hard fail 아님) — 아래 visual-harness 폴백 사다리 참조.
...
## 렌더 불가 환경 (visual-harness/browser 부재)
...
- **Layer-1 결정론 플로어는 그대로 실행** — visual-harness 경로를 이용해 headless 에서도 돌아간다:
  - `console.errors_zero`: `adapters/codex/bin/preflight.sh visual-harness <file.html>` 또는 adapter equivalent (Bash, playwright headless, adapter wrapper only)
...
- **링키지 (작업③ drill)**: `[det]` 항목의 visual-harness 경로가 drill (headless, `cases_growing/g8_design_verifier_breakage/`) 이 실제로 검증하는 플로어와 동일 코드다 — 드릴이 검증하는 결정론 플로어 = 프로덕션 폴백 플로어.
