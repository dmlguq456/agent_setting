...
(default on; `CODEX_DISTILL_ENABLE=0` opt-out). The `Stop` bridge first snapshots
and joins any stamped exact-`parent_sid` registered-child batch outside the
model, then emits one blocking continuation for typed harvest. A replay carrying
Codex's `stop_hook_active` flag exits silently instead of blocking or joining
again. With no such child it detaches `session-end` into the background and exits 0 with silent
stdout so distillation never consumes the turn-scoped timeout. The
...
| memory distill | Transcript delta extraction exists via `adapters/codex/bin/preflight.sh distill-delta <session-id>`. The user-facing `distill-propose` stays an explicit opt-in preview (reports `status=tool-contract`, exits 69 until `CODEX_DISTILL_ENABLE=1`). Automatic session-end and turn-nudge distillation is enabled by default: the `codex exec --sandbox read-only` worker is verified tool-free (see Distillation Boundary) and applies through `apply-distill-actions.py`; after Stop proves that no stamped interactive child remains open, it schedules session-end distillation detached; opt out with `CODEX_DISTILL_ENABLE=0` |
...
| dispatch-owner selection | An ordinary dispatch-depth-1 owner uses `dispatch-owner [--adapter <harness>] --dry-run|--register|--start`, a separate mapping row from `headless dispatch` below. It delegates to the portable `utilities/dispatch-owner.py` selector so `profiles/dispatch-defaults.yaml` and the SD-22 cascade (explicit target, then hard eligibility, then configured `depth1_owner`, then eligibility fallback) pick the adapter before any wrapper runs; the selector execs only the chosen `adapters/<selected>/bin/dispatch-headless.py` and never invokes the non-selected wrapper, so a Claude selection leaves no Codex registry/jobs row |
| headless dispatch | Run `preflight.sh headless --check <worktree>` before launch; it verifies native Skills, native Agents, and native Modes. Use `dispatch --dry-run|--register|--start [--require-hook-trust]` for registered work. Standard+ dispatch-depth-1 owners use `--completion-delivery auto`: a checked App Server probe selects an ephemeral same-thread supervisor, forced `supervised` fails before registration when unavailable, and explicit/unavailable fallback is reported as `poll-fallback`. Quick and dispatch-depth-2 workers stay one-shot `codex exec`. A direct detached depth-1 start bound to the actual interactive Codex thread forces authoritative current-hash hook trust before row/process creation and stamps `parent_completion_delivery=codex-stop-hook`; failure reports `child_spawned=0`. The wrapper validates the capability catalog's scalar `capability_mode`, validates an optional non-owner `worker_mode` through `mode-info`, and keeps behavioral `model_role` plus execution `model_profile` separate; `_kernel/owner` rejects a worker mode before prompt or registry writes and accepts its sealed owner profile without a role. The profile resolves through `config/models.conf`, caller model/reasoning replacement is denied, and substantive registered `mini` is denied. Rows expose resolved tier and profile granularity for Fleet. Registry writes and harvest rewrites are serialized with a `.lock` file. Registration materializes the portable kernel, one worker type, route metadata, and assigned Skill/unit. Approval, harvest, and cleanup contracts remain unchanged |
| parent completion park | The App Server supervisor owns exact-batch waiting and emits no model/tool activity between the owner's intermediate turn and join readiness. It resumes the same thread once per batch with child identities/status enums only, withholds intermediate `turn.completed`, and exposes exactly one final terminal handoff. Interactive Codex uses native Stop for stamped rows: successful spawn records the exact `parent_sid` attempt set in hashed pending state, Stop follows that set across terminal registry transitions, joins outside the model for a bounded interval, marks it delivered, and emits one `decision=block` continuation when exact harvest is ready. The current definition uses a 7200-second outer / 7140-second inner envelope. A stamped parent that predates that effective definition retains its cached 600-second outer timeout, so the current script detects the parent generation and caps its inner join at 540 seconds; this preserves typed recovery instead of letting Codex kill the hook first. A continuation carrying `stop_hook_active` exits silently, so neither a ready receipt nor the single timeout instruction can recursively block Stop. Its undelivered phase admits no model tool—including `dispatch-wait`/transport wait—and its delivered phase admits only exact-attempt `--status all` harvest; that native receipt is the sole park authority until exact harvest consumes it, so an unrelated same-session row cannot invert the lock. Runtime-error and absent/malformed terminal handoffs remain reported failures but do not make a delivered receipt unconsumable. The last harvest removes state. Registered-only `launch_claimed=0` rows never park; a finite timeout explicitly ends automatic delivery, requires later operator re-entry after completion, and never starts model/tool polling. A route-declared 2–4-way `parallel_group` uses one atomic `dispatch-batch --parallel-group`; the headless-owner undelivered phase admits only a manifest-bound one-missing-leg recovery with all N-1 peers proved and rejects direct member starts. Missing or invalid native state fails closed after spawn. The 300–600 second `preflight.sh dispatch-wait --attempt-id <id> --max <seconds>` path remains the explicitly reported fallback for unstamped/legacy rows; relative/absolute canonical wrapper paths and split/equals-style long options normalize to the same exact-attempt guard. Terminal live/unverifiable rows remain readiness/cleanup blockers but do not block unrelated tools. Native subagents remain outside this registry policy |
...
| hook invariants | `pretooluse-write-guard.py` enforces completion-mode-scoped parent parking before the existing write guards: supervised batch-registration/harvest phases and explicit polling fallback retain strict terminal-nonquiescent candidates, while native interactive Stop follows its hashed exact-attempt state across `open`/`running`/`done` and changes from no-tools to exact-attempt `--status all` harvest-only after delivery. `launch_claimed=0` rows are ignored and each valid harvest consumes one delivered receipt. `utilities/codex-parent-park.test.py` covers native Stop wait denial, terminal/delivered harvest, registered-only exclusion, canonical fallback aliases, path and option normalization, exact-attempt/range rejection, supervised wait denial, second-sibling admission, polling strictness, and terminal live/unverifiable recovery; `utilities/codex-stop-parent-park.test.py` covers Stop join/timeout/re-entry and the fast-terminal race. Session, prompt, permission, read-marker, design-check, native-subagent, and explicit preflight fallback mappings remain unchanged |
