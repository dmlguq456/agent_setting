...
| hook invariants | `pretooluse-write-guard.py` enforces completion-mode-scoped parent parking before the existing write guards: supervised batch-registration/harvest phases and explicit polling fallback retain strict terminal-nonquiescent candidates, while ordinary interactive mode selects only `open|running`. `utilities/codex-parent-park.test.py` covers canonical preflight/direct wait aliases, path and option normalization, exact-attempt/range rejection, supervised wait denial, second-sibling admission, delivered-batch harvest-only behavior, polling strictness, and interactive terminal live/unverifiable recovery. Session, prompt, permission, read-marker, design-check, native-subagent, and explicit preflight fallback mappings remain unchanged |
