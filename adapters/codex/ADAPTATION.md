...
| hook invariants | `adapters/codex/hooks/sessionstart-lifecycle.py` realizes SessionStart bootstrap context injection; `userprompt-lifecycle.py` realizes UserPromptSubmit mode/recall/briefing plus transition-only token-budget context and turn-nudge; `sessionend-lifecycle.py` realizes SessionEnd memory sync/distill and Stop detached session-end scheduling; `permissionrequest-lifecycle.py` is a registered no-op for Codex `PermissionRequest` (harness monitoring is owned by Codex native `/statusline`); `pretooluse-write-guard.py` matches supported local tools and realizes exact registered-child parent parking before write guards (artifact-order, git-state, core-first, memory-write, and the spec read gate for spec-changing artifacts); `posttooluse-read-marker.py` records actual spec/core reads through `PostToolUse`; `posttooluse-design-check.py` realizes design HTML console checks through `PostToolUse`; `utilities/codex-parent-park.test.py` replays the observed waste classes; run explicit preflight wrappers for events not covered by native hooks |
