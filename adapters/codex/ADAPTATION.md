...
| headless dispatch | Run `preflight.sh headless --check <worktree>` before launch; it verifies native Skills, native Agents, and native Modes. Use `dispatch --dry-run|--register|--start [--require-hook-trust]` for registered work. Standard+ dispatch-depth-1 owners use `--completion-delivery auto`: a checked App Server probe selects an ephemeral same-thread supervisor, forced `supervised` fails before registration when unavailable, and explicit/unavailable fallback is reported as `poll-fallback`. Quick and dispatch-depth-2 workers stay one-shot `codex exec`. A direct detached depth-1 start bound to an interactive Codex thread is always `parent_completion_delivery=poll-fallback`; it does not force hook trust or create Stop state. Capability/worker modes, model profiles, registry serialization, approval, harvest, and cleanup contracts remain unchanged |
| completion delivery | The App Server supervisor owns exact-batch waiting for a registered Codex headless owner and emits no model/tool activity between the owner's intermediate turn and join readiness. It resumes its owned thread with bounded child identity/status metadata and never injects raw output. Interactive Codex has no verified atomic idle claim or idempotency key, so Stop and PreToolUse do not simulate a wake: Stop is a silent no-op, there is no wildcard all-tool park, and the parent remains conversational. Completion is explicit finite operator fallback or later-turn harvest and is not reported as automatic resume. Legacy `codex-stop-hook` state is migration-only; exact terminal `--status all --attempt-id` harvest may atomically consume one pending or delivered receipt, while open/stale/foreign/broad selection fails. Claude parents continue to use Claude resume supervision; sidecars never own approvals |
