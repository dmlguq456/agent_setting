...
| hook invariants | `pretooluse-write-guard.py` enforces the supervised batch-registration/harvest phase state before the existing write guards; `utilities/codex-parent-park.test.py` covers second-sibling admission, delivered-batch harvest-only behavior, and polling fallback. Session, prompt, permission, read-marker, design-check, and explicit preflight fallback mappings remain unchanged |
