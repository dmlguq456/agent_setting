...
| native hook surface | `adapters/codex/hooks/hooks.json` registers Codex `SessionStart` lifecycle prep, synchronous `SessionEnd`, an explicit silent no-op `Stop` boundary, `UserPromptSubmit` prompt signals and turn nudges, `PermissionRequest` (registered no-op — harness monitoring is owned by Codex native `/statusline`), targeted `PreToolUse` material/write guards including the core-first adapter edit gate, `PostToolUse` spec/core read markers, and `PostToolUse` design HTML checks. Stop has no completion or lifecycle authority and no wildcard completion park is registered; explicit preflight remains fallback |
...
The `UserPromptSubmit` bridge extracts
