...
| native hook surface | `adapters/codex/hooks/hooks.json` registers Codex `SessionStart` lifecycle prep, `SessionEnd` memory sync/distill plus `Stop` detached session-end scheduling, `UserPromptSubmit` prompt signals and turn nudges, `PermissionRequest` (registered no-op — harness monitoring is owned by Codex native `/statusline`), all-local-tool `PreToolUse` parent parking plus write guards including the core-first adapter edit gate, `PostToolUse` spec/core read markers, and `PostToolUse` design HTML checks; explicit preflight remains fallback |
...
| headless dispatch | Tool-contract check: `adapters/codex/bin/preflight.sh headless --check <worktree>` verifies the worktree, `codex exec` availability, and installed Codex runtime projection (`agent-harness`, bootstrap, hooks, native Skills, native Agents, and native Modes). Add `--require-hook-trust` when headless work must prove complete Codex hook trust before launch. Use `adapters/codex/bin/preflight.sh dispatch --dry-run|--register|--start [--require-hook-trust] --worktree <path> --slug <slug> --capability <name> --mode <family/mode> --qa <quick|light|standard|thorough|adversarial>` to build the Codex command and register open jobs before launch. Dispatch keeps the active `CODEX_HOME` for auth/projection but does not choose a default model. The main/orchestrator must select model settings per job with `--model-role <portable-role>`, `--model <model> --reasoning <effort>`, or explicit `--inherit-model-settings`; simple jobs are downshifted by that caller decision. The wrapper passes noninteractive approval through `-c approval_policy=...` unless approval inheritance is explicit. The wrapper validates `capability-info`, `mode-info`, and the portable QA level before writing `.dispatch/jobs.log`; registry writes and harvest rewrites are serialized with a `.lock` file. `--register` and `--start` materialize the minimal typed worker prompt before appending the registry row; the prompt consumes wrapper-validated route metadata, the assigned Skill/mode, and write/verification guards without explicitly loading the full adapter bootstrap. `--start` reruns the same runtime projection check before launching; with `--require-hook-trust`, missing hook trust fails before registry writes. Use `adapters/codex/bin/preflight.sh liveness [jobs.log]` while waiting on dispatched work; it matches open jobs to Codex session JSONL files by `cwd` and transcript mtime. Use `adapters/codex/bin/preflight.sh harvest --slug <slug> --mark-done` after main-session harvest to mark registry rows done only; it does not merge or clean worktrees |
| parent parking | Once a registered child row opens, the exact depth-0 parent (`parent_sid`) or depth-1 conductor (`parent_attempt_id`) may use only `utilities/dispatch-wait.sh --attempt-id <id> --max 300..600`, continuation of that wait, and `preflight.sh harvest --attempt-id <id>`. The 600-second wait default keeps the parent model outside the polling loop. Raw child logs/transcripts, source/artifact/git inspection, redundant liveness, parallel work, short or wrong-attempt waits, and compound shell commands are blocked. `AGENT_PARENT_PARK_BYPASS=1` is an explicit operator recovery override, not an agent fallback |
...
| memory sync | Codex `SessionEnd` runs `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]`, which performs `mem sync` and then runs automatic distillation by default (the read-only `codex exec` worker is verified tool-free). Codex `Stop` schedules that lifecycle in a detached background process and returns silently before the hook timeout; opt out with `CODEX_DISTILL_ENABLE=0` |
...
default; opt out with `CODEX_DISTILL_ENABLE=0`), while `Stop` schedules the
same lifecycle in a detached background process so turn-scoped hook timeouts do
not block on distillation. The `UserPromptSubmit` bridge extracts
...
dispatch registry and parks a depth-0 parent by exact `parent_sid` or a depth-1
conductor by exact `parent_attempt_id`. While an owned registered row is open it
admits only the long exact-attempt wait, continuation of that wait, and typed
exact-attempt harvest; it rejects shell composition and every raw
...
coverage to other supported local tools after Codex records trust for that new
hook definition through `/hooks`.
