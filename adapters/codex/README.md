...
| native hook surface | `adapters/codex/hooks/hooks.json` registers Codex `SessionStart` lifecycle prep, `SessionEnd` memory sync/distill, `Stop` exact-session registered-child join plus detached session-end scheduling when no child is open, `UserPromptSubmit` prompt signals and turn nudges, `PermissionRequest` (registered no-op — harness monitoring is owned by Codex native `/statusline`), all-local-tool `PreToolUse` parent parking plus write guards including the core-first adapter edit gate, `PostToolUse` spec/core read markers, and `PostToolUse` design HTML checks; explicit preflight remains fallback |
...
| dispatch-owner selection | An ordinary dispatch-depth-1 owner launches through `preflight.sh dispatch-owner [--adapter <harness>] --dry-run|--register|--start`, a separate surface from the `headless dispatch` row below. It loads `profiles/dispatch-defaults.yaml` through `utilities/dispatch-owner.py`, runs the SD-22 cascade (explicit target, then hard eligibility, then configured `depth1_owner`, then eligibility fallback), and execs only the selected adapter's `dispatch-headless.py`; a Claude selection leaves no Codex registry/jobs row |
| headless dispatch | Tool-contract check: `adapters/codex/bin/preflight.sh headless --check <worktree>` verifies the worktree, Codex CLI/App Server availability, and installed runtime projection with native Skills, native Agents, and native Modes. Standard+ dispatch-depth-1 owners default to `--completion-delivery auto`: a successful `codex app-server --help` probe selects `app-server-supervised`; forced `supervised` fails before registration when unavailable, and `poll` or an unavailable auto probe reports `poll-fallback`. Quick/stage/review workers remain one-shot `codex exec`. The wrapper validates the capability catalog's scalar `capability_mode`, validates an optional non-owner `worker_mode` through `mode-info`, and keeps both separate; `_kernel/owner` rejects a worker mode before prompt or registry writes and is valid with its route-sealed owner profile alone. Route-bound jobs resolve `--model-profile deep|balanced-deep|light|mini` through `config/models.conf`, keep `model_role` independent, reject caller model/reasoning replacement, and reject substantive registered `mini`. Registry/Fleet rows emit capability/worker mode plus model profile, resolved tier, and profile granularity as separate fields. Registry writes and harvest rewrites are serialized with a `.lock` file. `--register` and `--start` materialize the minimal typed worker prompt. A direct detached depth-1 start bound to the actual interactive Codex thread automatically requires current-hash hook trust and fails with `child_spawned=0` before registry writes when it cannot be proved; its row records `parent_completion_delivery=codex-stop-hook`. Approval, route, liveness, harvest, and cleanup boundaries remain unchanged |
| parent completion park | Under `app-server-supervised`, the owner yields `runtime_wait: registered-children`; `utilities/dispatch_completion_join.py` waits outside the model for every exact `parent_attempt_id` child and the supervisor resumes the same ephemeral App Server thread once with a bounded typed receipt. A direct interactive parent uses the same joiner through native Stop: a stamped depth-1 spawn first records its exact attempt in hashed pending state, then the parent ends the model turn. Stop follows that immutable set even after an orphan watcher changes a row to `done`. A parent proved current for the 7200-second Stop definition receives a 7140-second inner join; an already-stamped parent that predates the effective definition is capped at 540 seconds so its cached legacy 600-second outer timeout cannot kill the hook before typed recovery. Stop then publishes the delivered phase and returns one bounded continuation for exact-attempt `--status all` harvest. Before delivery all model tools—including `dispatch-wait` and transport waits—are blocked; after delivery only exact typed harvest is admitted, and each valid harvest consumes its receipt. A `launch_claimed=0` registration never parks. A finite timeout states that automatic delivery ended and operator re-entry is required rather than promising another polling loop. Intermediate turns never become terminal JSONL events. A declared 2–4-way `parallel_group` starts through one atomic `dispatch-batch --parallel-group`; the headless-owner phase policy admits exactly one missing-leg recovery only after all N-1 peers are manifest-bound, and rejects direct member starts. Raw child output, source/artifact/git inspection, redundant liveness, unrelated work, and compound shell commands remain blocked. `poll-fallback` retains the long exact `preflight.sh dispatch-wait --attempt-id <id> --max 300..600` command for unstamped/legacy rows; both split and `--arg=value` long options are accepted. Terminal live/unverifiable rows still fail readiness/cleanup without freezing unrelated tools. Native Codex subagents are not registered-child candidates. `AGENT_PARENT_PARK_BYPASS=1` remains an explicit operator recovery override |
...
| memory sync | Codex `SessionEnd` runs `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]`, which performs `mem sync` and then runs automatic distillation by default (the read-only `codex exec` worker is verified tool-free). Codex `Stop` first owns any stamped interactive registered-child join; only when none remains open does it schedule the same lifecycle in a detached background process and return silently. Opt out with `CODEX_DISTILL_ENABLE=0` |
...
| agent-initiated memory retrieval | Codex prompt hooks do not classify prompts for recall. When the agent decides prior context is relevant, run `adapters/codex/bin/preflight.sh recall <query> [cwd] [session-id]`; retrieval stays project-scoped by default |
...
sandbox decisions. The `PreToolUse` bridge matches all supported local tools.
Before its existing write/edit/multiedit/patch checks, it resolves the canonical
dispatch registry and parks a dispatch-depth-0 parent by exact `parent_sid` or a dispatch-depth-1
...
admits only another exact same-parent dispatch start, while a delivered batch
admits only exact harvest. Both reject shell composition and every raw
inspection/work path. With no owned row open, qualified
`functions.apply_patch` payloads and other writes continue through
...
disclosed prompt/conformance fallback rather than total enforcement. A later
`write_stdin` continuation does not rerun the pre-hook after the originating
