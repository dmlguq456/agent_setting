...
| memory sync | Codex `SessionEnd` and `Stop` hook bridges run `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]`, which performs `mem sync` and then runs automatic distillation by default (the read-only `codex exec` worker is verified tool-free); the bridge keeps stdout empty so Stop hook output never violates Codex JSON parsing; opt out with `CODEX_DISTILL_ENABLE=0` |
...
write redirects, direct `spec/prd.md` reads, and design HTML save paths. Treat
target-ambiguous shell reads/writes to guarded paths as an explicit tool
contract: run `preflight.sh write`, `preflight.sh read`, or
