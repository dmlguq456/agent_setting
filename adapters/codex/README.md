...
| headless dispatch | Tool-contract check: `adapters/codex/bin/preflight.sh headless --check <worktree>` verifies the worktree, Codex CLI/App Server availability, and installed runtime projection with native Skills, native Agents, and native Modes. Standard+ dispatch-depth-1 owners default to `--completion-delivery auto`: a successful `codex app-server --help` probe selects `app-server-supervised`; forced `supervised` fails before registration when unavailable, and `poll` or an unavailable auto probe reports `poll-fallback`. Quick/stage/review workers remain one-shot `codex exec`. The wrapper validates capability/worker modes and route-sealed model profiles, rejects substantive registered `mini`, and serializes registry/harvest writes. A direct detached depth-1 start bound to an interactive Codex thread is always explicit `parent_completion_delivery=poll-fallback`; it neither forces hook trust nor creates native Stop state. Approval, route, liveness, harvest, and cleanup boundaries remain unchanged |
| completion delivery | Under `app-server-supervised`, a registered headless owner yields `runtime_wait: registered-children`; `utilities/dispatch_completion_join.py` waits outside the model for every exact `parent_attempt_id` child and the supervisor resumes its owned ephemeral App Server thread with one bounded typed receipt. Interactive Codex is intentionally different: current App Server `turn/start` has no verified atomic idle claim or idempotency key, so Stop and PreToolUse do not simulate auto-resume. The parent stays conversational while the child runs and completion is explicit finite operator fallback or later-turn harvest. Existing `codex-stop-hook` state is migration-only: exact terminal `--status all --attempt-id` harvest may consume one pending/delivered receipt, while open/stale/foreign/broad selection cannot. Raw child output and synthetic user/developer messages never enter the main transcript. Native Codex subagents remain a separate surface |
