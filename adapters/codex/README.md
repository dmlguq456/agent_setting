...
| memory sync | Codex `SessionEnd` and `Stop` hook bridges run `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]`, which performs `mem sync` and then runs automatic distillation by default (the read-only `codex exec` worker is verified tool-free); the bridge exits 0 with silent stdout on success so helper output never violates Stop hook parsing; opt out with `CODEX_DISTILL_ENABLE=0` |
