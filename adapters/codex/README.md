...
| loop guidance | `adapters/codex/bin/preflight.sh loop-info <oncall|note|study|drill|runtime-watch>` distinguishes the shared external runner from Codex-native scheduling. `oncall`/`study` and user-started `drill` default to usage-aware `auto` selection between Claude/Codex; drill remains manual-only from the Codex UI. `note` remains an external scheduler loop while the related `autopilot-note` capability is available on demand through Codex-native Skill/plugin projections |
