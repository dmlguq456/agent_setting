#!/usr/bin/env sh
# Codex adapter-owned material PDF extraction launcher.
set -eu
...
Checks or extracts text from a PDF through a Codex-owned material
...
if [ "$#" -eq 0 ]; then
  ...
  printf 'tool_contract_check=adapters/codex/bin/preflight.sh pdf-extract --check <file.pdf>\n'
...
[ "$#" -ge 1 ] || { echo "codex pdf-extract: PDF path required" >&2; exit 64; }
pdf=$1
shift
out=""
if [ "${1:-}" = "--out" ]; then
  [ "$#" -eq 2 ] || { echo "codex pdf-extract: --out requires one output path" >&2; exit 64; }
  out=$2
  shift 2
fi
[ "$#" -eq 0 ] || { echo "codex pdf-extract: unexpected arguments: $*" >&2; exit 64; }
