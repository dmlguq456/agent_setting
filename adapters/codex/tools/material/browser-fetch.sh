#!/usr/bin/env sh
# Codex adapter-owned material browser-fetch launcher.
set -eu
...
Checks or fetches a rendered web page through a Codex-owned material
...
if [ "$#" -eq 0 ]; then
  ...
  printf 'tool_contract_check=adapters/codex/bin/preflight.sh browser-fetch --check <url>\n'
