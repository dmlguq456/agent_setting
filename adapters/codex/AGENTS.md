...
- Use `adapters/codex/bin/preflight.sh headless [--check] <worktree>` before any Codex headless dispatch. Do not launch headless work unless the job is main-dispatched, depth 1, registered in `.dispatch/jobs.log`, and monitored with `adapters/codex/bin/preflight.sh liveness [jobs.log]` while waiting.
