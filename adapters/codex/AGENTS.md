...
- Use `adapters/codex/bin/preflight.sh ui-info` to inspect the Codex UI boundary. Codex supports native `/statusline` and `/title` built-in item configuration, but not a Claude-style arbitrary live statusline script; harness-specific signals stay in `preflight.sh status` and hook `statusMessage` output.
...
- Use `adapters/codex/bin/install-runtime-projection.sh [--install-plugin]` to wire `$CODEX_HOME` (default `$HOME/.codex`) to the harness projection (`agent-*` pointers, `hooks.json`, native skills/agents); it is idempotent and never touches Codex credentials, sessions, logs, caches, or DBs. Use `adapters/codex/bin/check-runtime-projection.sh` for a read-only `status=ok|failed` validation of that wiring.
