...
- For memory behavior, apply `core/MEMORY.md` §7.4 recall-first: before style, format, naming, figure/table/report-layout, or prior-convention decisions, run `adapters/codex/bin/preflight.sh recall "<query>" [cwd]` or `tools/memory/recall.sh` directly. For deep memory-scout work, use a read-only Codex subagent/procedure only after `subagent-info --check`, and return the fixed <=15 line verdict; no memory or file writes.
