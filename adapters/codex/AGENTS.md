...
- Codex `SessionEnd` runs `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]` for memory sync plus automatic distillation (enabled by default; opt out with `CODEX_DISTILL_ENABLE=0`). Codex `Stop` launches the same session-end lifecycle in a detached background process and exits 0 with silent stdout so turn-scoped Stop hook timeouts are not consumed by distillation; run `session-end` manually when hooks are unavailable.
