...
- Codex `UserPromptSubmit` hook bridge runs `adapters/codex/bin/preflight.sh mode [cwd] [session-id]` (the one-line tracked/untracked routing anchor), plus `adapters/codex/bin/preflight.sh recall "<prompt>" [cwd]` and `adapters/codex/bin/preflight.sh briefing [cwd]` when they have content, and the `adapters/codex/bin/preflight.sh turn-nudge [cwd] [session-id]` side effect, then emits one `hookSpecificOutput.additionalContext` — the mode anchor line, matching Claude Code's per-turn footprint (no routing-contract/git-risk aggregate). `adapters/codex/bin/preflight.sh prompt-signal [cwd] [session-id]` is a worker-startup/manual subcommand (full routing contract plus git dirty/worktree/dead-branch risks re-derived from the harness status snapshot), not a per-turn injection; run it — like `status` — manually or at worker startup when you need that signal.
