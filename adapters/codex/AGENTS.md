...
- Shell/Bash/`functions.exec_command` reads and writes have targeted hook coverage for obvious write redirects, common mutation commands (`tee`, `touch`, `cp`, `mv`, `rm`), direct `spec/prd.md` reads, and design HTML save paths. Before target-ambiguous shell I/O touches guarded paths, run the matching explicit `preflight.sh write`, `preflight.sh read`, or `preflight.sh design` wrapper.
