...
- Shell/Bash/`functions.exec_command` reads and writes have targeted hook coverage for obvious write redirects and direct `spec/prd.md` reads. Before target-ambiguous shell I/O touches guarded paths, run the matching explicit `preflight.sh write`, `preflight.sh read`, or `preflight.sh design` wrapper.
