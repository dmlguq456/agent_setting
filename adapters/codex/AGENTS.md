...
- Codex `SessionEnd` hook bridge runs `adapters/codex/bin/preflight.sh session-end [cwd] [session-id]` for memory sync plus opt-in distill proposal; run it manually when hooks are unavailable.
...
- Use `adapters/codex/bin/preflight.sh distill-delta <session-id>` to inspect Codex transcript deltas. `adapters/codex/bin/preflight.sh distill-propose <session-id> [cwd]` reports `status=tool-contract` and exits 69 until `CODEX_DISTILL_ENABLE=1` is explicit; do not auto-apply memory distillation until a Codex no-tools worker contract exists.
