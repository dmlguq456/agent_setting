...
- **Harness integration:** automatic memory lifecycle belongs only to the interactive, user-facing main session (D-42). `mem inject --hook` may expose a bounded working, durable, and profile summary at main SessionStart; adapters whose start event repeats on resume, clear, or compact may keep this opt-in. Main SessionEnd runs current-project `mem sync` plus bounded `MEM_DUMP_PUSH=1` mirroring under D-31. Main SessionEnd and turn-counter triggers may launch a no-tools distiller agent through `mem-distill-dispatch.sh`; automatic additions must declare exactly one storage purpose: `decision`, `user-correction`, `unresolved-obligation`, or `artifact-pointer`. Artifact-backed knowledge is represented by a pointer and retrieval reason, not a duplicate artifact summary. Registered dispatches, loops, title workers, distillers/curators, and native subagents are worker sessions: they never inject memory automatically, advance distill counters, sync on exit, or launch a curator. Every automatic model-backed distill path retains the D-41 bounded-worker controls. Prompt-submit hooks do **not** classify every prompt for recall; work intake instead records an explicit contextual `recall` or `skip` decision through `mem recall-gate`. `builtin-memory-guard.sh` keeps DB writes on the unified `mem` path. Hook registration remains adapter-native.
- **Recall:** `mem recall` searches active retrieval capsules first, then body/CJK compatibility indexes, with optional exact `--topic`; historical rows require `--include-superseded`. If the ID is already known, use `mem show <id>`. At material work intake the acting agent records `recall` or `skip` with a short reason through `mem recall-gate`; this records no raw prompt and does not restore prompt-wide automatic recall.
...
`mem inject` may provide a bounded SessionStart summary of active working, durable, and profile records. At work intake the acting main agent explicitly decides whether prior context could materially help. Record `recall` with a focused query or `skip` with a short contextual reason through `mem recall-gate`; this is an opportunity gate, not a semantic prompt classifier. Retrieval is information access, not handoff consumption. Bounded telemetry stores the gate id, decision, reason, query hash, result count, and later `applied|miss` outcome without storing raw prompts.
...
**Agent-initiated recall:** the agent decides whether prior context may materially improve the current judgment and must not silently omit that decision at work intake. No fixed signal words, mandatory topic list, prompt classifier, or category-to-recall rule substitutes for contextual judgment. Once the agent chooses to recall, search active curated memory first, widen to historical, cross-project, or raw-session data only when useful, and cross-check retrieved claims against current code or artifacts because memory can be stale.
...
- `mem recall-gate` makes that choice observable at work intake and executes a chosen recall; it does not inspect or classify the prompt.
