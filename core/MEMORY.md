...
> **06 (대화 메타 규칙)** 은 매트릭스 제외 — runtime adapter bootstrap 의 응답 규율이 단일 source (메인 에이전트 전용; sub-agent 는 사용자와 직접 대화 X. Claude adapter: `adapters/claude/CLAUDE.md` §1~§3). `/post-it --scope user` 의 default collab 저장처로 `06_collaboration_style` 레코드 자체는 유지. **07 (코드)** 은 개발팀·기획팀·메인 에이전트만 — 편집팀(wording 영역) 제외. agent 별 3–5 aspect 참조 default (2026-05-26 정리).
