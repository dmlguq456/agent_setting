...
There are two retrieval surfaces. First, curated memory in durable and working tiers is searched by `mem recall` through SQLite FTS5 with BM25 ranking, falling back to LIKE or `rg`. If a snippet is insufficient, immediately use `show <id>` or `recall --full --limit N`. Direct SQLite or `dump.jsonl` queries are not a normal retrieval path. Second, `--sessions` searches raw historical transcripts that have not been distilled into memory. Raw sessions are noisy and should supplement curated memory only when useful.
