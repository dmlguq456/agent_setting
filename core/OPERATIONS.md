...
2. **Execution and naming:** create the worktree with `git worktree add <path> -b <slug> origin/<base>` using §5.9 base selection. The sole canonical path is the sibling directory `<repo>-wt/<slug>`, such as `Foo-wt/<slug>` for `Foo`; do not invent `<repo>_worktrees/`. `worktree-path-guard` hard-enforces this naming for `git worktree add`, while untracked mode, non-add subcommands, and non-git contexts fail open.
   ...
   - **Harness-neutral launch broker under SD-51~53:** depth 0 prepares one deterministic, non-model broker before a standard+ conductor starts and passes its immutable endpoint, instance identity, and canonical jobs registry to the conductor. Every logical depth-2 headless hop — same-harness and cross-harness alike — is a declarative broker request; the conductor never relies on recursive native-subagent or CLI process spawning. The broker validates a versioned allowlisted envelope, records an atomic idempotent request state, registers the attempt before spawn, executes exactly the harness selected by the route layer, and never reroutes. Logical `depth=2,parent=<conductor>` remains independent of OS ancestry. A missing, stale, mismatched, or unfenced broker fails closed without caller-local launch. Broker request completion does not relax the conductor's same-turn `dispatch-wait` and harvest obligation for the launched model worker.
