...
2. **Execution and naming:** create the worktree with `git worktree add <path> -b <slug> origin/<base>` using §5.9 base selection. The sole canonical path is the sibling directory `<repo>-wt/<slug>`, such as `Foo-wt/<slug>` for `Foo`; do not invent `<repo>_worktrees/`. `worktree-path-guard` hard-enforces this naming for `git worktree add`, while untracked mode, non-add subcommands, and non-git contexts fail open.
   ...
   - **Nested child-spawn eligibility under SD-48:** root headless readiness, native-subagent readiness, and a conductor's ability to launch a registered depth-2 child are separate runtime surfaces. Before compiling a route with depth-2 nodes, bind checked evidence for `(parent_harness,parent_transport,parent_sandbox,child_harness,launch_authority)`. Only `supported` is eligible; `unknown` and `unsupported` fail closed. Dispatch contract v3 requires `launch_authority=conductor`; logical ownership remains `depth=2,parent=<conductor>` and matches the process that invokes the adapter wrapper. Supplementing a `--start` invocation with the checked evidence flags obtained from the documented nested-headless probe is the required procedure, not a gate bypass; workers proceed without re-confirmation even when a caller-provided argument list omitted those flags. `dispatch-node.py` forwards the record's checked `dispatch_evidence` tuple to the wrapper automatically (an explicit flag equal to the record passes; a mismatch fails loud), and on a depth-2 `--start` with no evidence supplied the adapter wrapper runs the eligibility probe itself and binds only a successfully completed probe's checked result — explicit evidence, even `unknown`, is never overwritten, and non-`supported` outcomes keep failing closed.
   ...
   - **Direct headless launch under SD-61~63:** dispatch contract v3 has no resident launch broker, request spool, heartbeat, lease, or fencing identity. A standard+ depth-1 conductor invokes the checked adapter wrapper directly for every same- or cross-harness depth-2 attempt. The canonical registry lock atomically claims the stable attempt identity before spawn; a duplicate claim never creates another child. A Codex depth-1 capability owner running with `workspace-write` receives `sandbox_workspace_write.network_access=true`, `AGENT_NESTED_HEADLESS_NETWORK=1`, and a worktree-local writable `CODEX_HOME`; that home links existing auth/config read-only and keeps mutable nested session state inside the owner sandbox. Depth-2 workers do not inherit the network widening. Contract-v1/v2 route and broker state are read-only migration inputs. The broker utility may expose diagnostic `status` and idempotent `stop` for one compatibility release, but production dispatch never calls `ensure`, `request`, or `serve`.
   ...
   - **One-shot wait contract under SD-14:** a headless main, including a conductor, exits when its turn ends. A conductor therefore must not end a turn waiting for a notification after dispatching a stage. It polls within the same turn through `utilities/dispatch-wait.sh`, then harvests. Enforcement combines injected wrapper guidance and the deterministic wait helper. A Claude Stop-hook gate remains disabled because it does not fire reliably under `claude -p` and can corrupt result output. Every `standard+` conductor prompt states this at the top: no asynchronous Monitor/wakeup/scheduling waits; poll synchronously with `utilities/dispatch-wait.sh` in the current turn until terminal, then harvest. This standard top-of-prompt clause (SD-71) is an auxiliary layer only — it reinforces but never substitutes for the runtime tool policy an adapter can enforce (for example, denying proven-fatal async wait tools) or for the post-exit orphan reconcile below.
   - **Post-exit orphan reconcile under SD-64/71:** a conductor can die mid-pipeline (session end, crash, limit) leaving live children or unstarted successor nodes orphaned. The deterministic classification is: exact conductor attempt death (`pid`+`pid_start` mismatch or gone) AND at least one route completion node without a marker AND (an open/live child OR an un-started successor node whose predecessors are all marked). When all three hold, reconcile closes the conductor row `note=dead-parent-orphaned` and surfaces the route id and resume boundary (the first incomplete node) on liveness, adapter preflight status, and the Fleet current-attempt view. This is detection and surfacing only: no automatic replacement, resume, or restart, and no closing of a live child — resuming an orphaned route is a depth-0 decision. Live conductors, live children, and routes with every node marked never classify as orphaned; classification fails closed (never claims `dead-parent-orphaned`) when the route record cannot be read safely.
   - **Codex linked-worktree mutation is a no-commit worker under SD-69:** a Codex sandbox running `workspace-write` in a linked task worktree keeps `.git` and the resolved git-common-dir protected even when other roots are writable; widening the sandbox to expose Git metadata is never an accepted fix, and a stage must never claim a commit happened when the sandbox made none. A linked-worktree Codex mutation stage therefore produces source diff, tests, and evidence but does not `git commit`; the route's `source_commit` holds unmoved until stage end, and a trusted depth-0 or Claude boundary commits after the stage's own PASS gate and confirms diff attribution before doing so. Such a worker's only writable roots beyond the task worktree and the canonical artifact root are the exact primary `$AGENT_HOME/.spec-grounding` directory (created safely if absent) — never all of agent home, and never `.git`.
   - **Completion marker bound to the exact attempt row under SD-70:** completing a node takes the canonical registry (`jobs.log`) path and the current exact attempt id, not just the route/node pair. It writes the completion marker atomically first, then idempotently closes only that one attempt row `done note=completed-marker` with the marker as evidence — it never breadth-closes a prior `BLOCKED` row or a later live retry of the same node. Marker write and row close are each idempotent under retry. If the row close fails after the marker is written, the marker is preserved and the command returns a structured nonzero rather than silently succeeding or discarding evidence; reconcile later repairs only that exact marker-backed stale row, never any other row for the route/node.
