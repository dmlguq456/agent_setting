...
2. **실행** — worktree 생성 (`git worktree add <path> -b <slug> origin/<base>`, base 선정은 §5.9) 후 두 모드. **`<path>` 명명 규칙 (canonical, 2026-06-12 사용자 확정)**: 형제 디렉토리 `<repo>-wt/<slug>` 로 판다 (예: repo 가 `…/Foo` 면 worktree 는 `…/Foo-wt/<slug>`). Adapter UI/status surface 가 이 규칙을 신호로 삼을 수 있으므로 `<repo>_worktrees/` 같은 변형 금지, **`-wt/` 단일 표준**. (harness: `worktree-path-guard` hook 이 이 경로 컨벤션을 hard 강제 — 내장 worktree 툴(repo 안 기본 경로)과 `<repo>-wt/` 밖 `git worktree add` 를 deny; `.untracked`·비-add 서브커맨드·비-git 은 fail-open. 2026-07-10 drill g3/g6.)
   ...
   - **상시 loop adapter 선택 (2026-07-13, 사용자 token 고갈 사고)**: `drill`·`oncall`·`study` 같은 외부 loop runner도 ⑧과 같은 `utilities/usage-check.sh` 상태와 **한 canonical jobs.log** 를 사용하고, 각 실제 runtime attempt를 같은 registry에 `open→done`으로 등록한다. 기본은 `auto`: 알려진 `limited`를 최우선 회피하고, 한쪽만 `ok`면 그쪽, 둘 다 `ok|unknown`이면 명시 `HARNESS_CAPACITY_BIAS=claude|codex`를 먼저 따르며 bias가 `auto`면 날짜+loop key의 결정론적 중립 분산으로 Claude/Codex를 고른다. 둘 다 `limited`면 새 세션을 열지 않고 unavailable로 끝낸다. 실행 중 새 session/usage limit이 드러나면 해당 loop는 `done` limit marker(`reset=` 포함)를 canonical registry에 기록하고, `auto` 실행에 한해 반대 하네스로 한 번 즉시 failover한다. `DRILL_ADAPTER`/`--adapter`와 `LOOP_ADAPTER`의 명시 `claude|codex|opencode`는 usage routing보다 우선하는 강제 선택이며 OpenCode는 명시 선택만 지원한다. 정확한 잔여 quota API가 없는 동안 `auto`는 **known-limit 회피+중립 분산**이지 잔여 퍼센트 추정이 아니다; “거의 소진”처럼 limit 전 사용자 관측은 `HARNESS_CAPACITY_BIAS`로 전달한다. 이 선택은 runtime만 바꾸며 token pressure가 stage graph·검증·guard를 줄이지 못한다는 비간섭 계약은 그대로다.
