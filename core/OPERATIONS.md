...
2. **Execution and naming:** create the worktree with `git worktree add <path> -b <slug> origin/<base>` using §5.9 base selection. The sole canonical path is the sibling directory `<repo>-wt/<slug>`, such as `Foo-wt/<slug>` for `Foo`; do not invent `<repo>_worktrees/`. `worktree-path-guard` hard-enforces this naming for `git worktree add`, while `WORKTREE_GUARD_BYPASS=1`, non-add subcommands, and non-git contexts fail open.
   ...
   - **Post-exit orphan reconcile under SD-64/71:** a conductor can die mid-pipeline (session end, crash, limit) leaving registered children or unstarted successor nodes orphaned. Since a dispatch-depth-1 owner is normally registered before route compilation, its route context is derived deterministically from exact child rows in the same repo/worktree, including terminal children; conflicting route tuples fail closed. The deterministic classification is: exact conductor attempt death (`pid`+`pid_start` mismatch or gone) AND at least one route completion node without a marker AND (any open child row OR an un-started successor node whose predecessors are all marked). Each registered dispatch-depth-1 owner launch starts one non-model watcher bound to its exact PID/start-time and attempt; it exits when the row is already terminal, or after owner exit revalidates and closes only a true orphan. This avoids a polling daemon and does not consume a model-worker slot. Reconcile closes the conductor row `note=dead-parent-orphaned` and surfaces the route id and resume boundary (the first incomplete node) on liveness, adapter preflight status, and the Fleet current-attempt view. This is detection and surfacing only: no automatic replacement, resume, or restart, and no closing of a child — resuming an orphaned route is a dispatch-depth-0 decision. Live conductors and routes with every node marked never classify as orphaned; classification fails closed (never claims `dead-parent-orphaned`) when route context conflicts or the route record cannot be read safely.
