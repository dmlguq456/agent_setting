...
2. **Execution and naming:** create the worktree with `git worktree add <path> -b <slug> origin/<base>` using §5.9 base selection. The sole canonical path is the sibling directory `<repo>-wt/<slug>`, such as `Foo-wt/<slug>` for `Foo`; do not invent `<repo>_worktrees/`. `worktree-path-guard` hard-enforces this naming for `git worktree add`, while `WORKTREE_GUARD_BYPASS=1`, non-add subcommands, and non-git contexts fail open.
   ...
   - **One-shot wait and parent-parking contract under SD-14:** a headless main, including a conductor, exits when its turn ends. Once it registers a child, the parent is parked until every exact child attempt it owns is closed: it may invoke only `utilities/dispatch-wait.sh --attempt-id <id> --max 300..600`, the continuation of that same wait process, and exact-attempt typed harvest. It must not read child transcripts/logs/artifact bodies, inspect source or git, run redundant liveness probes, or perform parallel work while parked. The wait helper defaults to its 600-second cap so the parent model remains outside the loop instead of re-entering every few seconds. Parent ownership is exact: depth 0 matches `parent_sid=<session-id>` and a depth-1 conductor matches `parent_attempt_id=$AGENT_DISPATCH_ATTEMPT_ID`; a foreign or terminal row never parks the caller. An adapter with a pre-tool interception surface must deny non-control local tools and compound-shell escapes deterministically; a surface it cannot intercept is a declared checked fallback, never silent parity. A Claude Stop-hook gate remains disabled: the current `claude -p` probe fires Stop, but blocking loops instead of reliably stopping and stdout is not preserved, so it cannot safely gate completion. Every `standard+` conductor prompt carries the same parking clause. Post-exit orphan reconcile remains mandatory and independent.
