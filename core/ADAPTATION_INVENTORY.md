...
| Codex native hook surface | `adapters/codex/hooks/hooks.json`, `adapters/codex/hooks/pretooluse-write-guard.py` | adapter-native projection | Codex `PreToolUse` command hook bridge for write/edit/patch guard enforcement; projected as `codex_setting/codex-hooks`. Do not reuse Claude hook files or `settings.json` hook payloads as Codex-native output. |
