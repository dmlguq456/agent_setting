...
| Agent notes root | `<agent-notes-root>` (current local realization: `/home/nas/user/Uihyeop/notes/`) | runtime/continuity state | Portable docs define the layer and required queues; data is not harness source and must not be committed here. |
| Worklog board app | `<worklog-board-app>` (prefer `$AGENT_STATE_HOME/worklog-board` or another path outside `<agent-home>`; current legacy Claude realization may be `$HOME/.claude/worklog-board`) plus `<worklog-board-app>-wt/` worktrees | local app workspace / needs-split if promoted | Treat current code, DB/cache, build output, dispatch logs, env files, and worktrees as external to this harness. If promoted later, split source into a separate app repo or portable tool first. `<agent-home>/worklog-board*` is gitignored local fallback, not source. |
