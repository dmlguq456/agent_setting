...
| Headless dispatch contract | Codex/OpenCode `preflight.sh headless` | adapter-native tool-contract | Reports runtime-native headless command templates (`codex exec`, `opencode run`) and checks candidate worktrees without launching. Full unattended dispatch remains gated on adapter-native transcript liveness mapping; do not reuse Claude headless flags or session paths. |
