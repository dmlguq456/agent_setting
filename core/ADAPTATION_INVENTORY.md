...
| Codex native plugin/command surface | not yet materialized | needs-split | If Codex needs plugin or command-like entrypoints, generate adapter-owned output from `capabilities/` and `roles/`; do not reuse Claude command, hook, or Skill files as Codex-native output. |
| OpenCode native skill surface | `adapters/opencode/skills/*/SKILL.md` | adapter-native generated projection | Generated from `capabilities/*.md` by `adapters/opencode/bin/sync-native-skills.py`; projected as `opencode_setting/opencode-skills`. Do not reuse Claude Skill files or rely on the `~/.claude/skills/` autoload compat path. |
...
| OpenCode native command/plugin surface | not yet materialized | needs-split | OpenCode has native `.opencode/command/` and JS/TS plugin hook surfaces. Generate adapter-owned output from `capabilities/` and `roles/`; do not reuse Claude command, agent, or hook files as OpenCode-native output. |
