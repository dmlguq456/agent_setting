...
| OpenCode native command/agent/plugin surface | not yet materialized | needs-split | OpenCode has native `.opencode/command/`, `.opencode/agent/`, and JS/TS plugin hook surfaces. Generate adapter-owned output from `capabilities/` and `roles/`; do not reuse Claude command, agent, or hook files as OpenCode-native output. |
