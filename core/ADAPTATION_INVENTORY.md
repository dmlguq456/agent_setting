...
| Codex command-like surface | not yet materialized | needs-split | If Codex needs command-like entrypoints beyond native skills/plugins, generate adapter-owned output from `capabilities/` and `roles/`; do not reuse Claude command or hook files as Codex-native output. |
