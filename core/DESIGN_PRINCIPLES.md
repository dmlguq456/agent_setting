...
- Every review that runs is adversarial in stance regardless of intensity: it tries to falsify the artifact, names at least one concrete failure mode, and treats inadequate evidence as not proven. This stance is universal and near-free; it is distinct from a separate adversary pass, which only higher tiers add (`CONVENTIONS §1.1`).
- Independent QA runs where intensity and risk call for it, and its independence axis is a different harness or model family (Claude ↔ Codex/GPT): a cross-harness 2-way independent replica whose results are merged at the strongest risk point from strong, additional bounded perspectives at thorough, and external-adversary/security/claim verification at adversarial. Standard may open the same replica at a risk-bearing point; direct and quick stay single-session but keep the adversarial stance. When only one harness is available, fail loudly if the cross-harness pass was explicitly requested; otherwise fall back to a same-harness independent session and report the reduced independence.
