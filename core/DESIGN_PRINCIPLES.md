...
- Waiting and harvesting are part of the deterministic flow only when the parent runtime owns a checked session supervisor: registered standard+ conductors yield while that supervisor joins the exact child batch outside the model, then resume with a bounded typed receipt. An interactive runtime without atomic idempotent continue-if-idle remains conversational; hooks must not simulate wake by blocking Stop, parking all tools, or creating a synthetic user turn. `dispatch-wait`/later-turn harvest is a disclosed finite fallback, not parity. `OPERATIONS §5.10` owns the runtime details.
