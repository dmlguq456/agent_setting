...
| native subagent default model | `hooks/subagent-model-default.sh` | `adapter-payload-wrapper` | Native subagent spawns carry the adapter-config-declared default model tier instead of silently inheriting the interactive session model (core/ADAPTATION.md §3). An explicit per-invocation model choice, an agent-definition frontmatter pin, and an intentional parent-inherit surface (fork-style spawn) always win; undecidable input fails open to inheritance, silently. | Realize the same config-declared default through the runtime's native subagent model configuration surface, or record the surface as unsupported; do not consume the Claude PreToolUse payload shape as configuration. |
| conductor Stop gate | `hooks/conductor-stop-gate.sh` | `portable-check` (**UNREGISTERED — fallback only**) | Legacy SD-14b fallback for a runtime without the SD-78 session supervisor. It blocks a conductor turn-end with open children and directs exact `dispatch-wait`/harvest recovery. **Held / not registered** because Claude print-mode Stop does not preserve a reliable result stream. | Use only for an explicitly reported polling fallback; never register it as the normal supervised completion path. |
| registered-child completion park | adapter session supervisor + native pre-tool policy | `adapter-coupled-automation` | A standard+ owner yields after registering a batch; the supervisor joins the exact `parent_attempt_id` batch outside the model and resumes the same session once with a bounded typed receipt. Its atomic attempt-scoped phase state allows only additional exact same-parent dispatch starts while the new batch is undelivered, then only exact harvest for delivered pending rows. Model-issued waits, raw inspection, liveness, unrelated work, and shell composition are denied in both phases; missing state is recovery-only harvest. An explicit polling fallback preserves the same terminal-nonquiescent park. The post-exit watcher reconciles every exact owner PID termination, including model/auth/limit failure before any child exists; an `open` registry row alone is never liveness evidence. Outside those registered completion-delivery modes, an ordinary interactive parent parks only exact latest `open|running` rows; terminal live/unverifiable rows remain fail-closed for readiness and cleanup but cannot become a session-wide local-tool lock. Runtime-native subagents are a separate surface and create no registered park candidate. | Probe Codex App Server or Claude session resume before launch. Realize the phase policy in the runtime's native pre-tool surface without mutating user configuration. If unavailable, report the checked `dispatch-wait` fallback explicitly; arbitrary detached output never claims auto-resume. |
