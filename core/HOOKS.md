...
| registered-child completion delivery | adapter-owned session supervisor | `adapter-coupled-automation` | A registered headless owner yields after registering a batch; its runtime supervisor joins the exact `parent_attempt_id` batch outside the model and resumes the same owned session once with a bounded typed receipt. Atomic attempt-scoped phase state admits only checked sibling registration before delivery and exact harvest afterward. Model-issued waits, raw inspection, liveness prose, unrelated work, and shell composition are denied inside that supervised owner. A general interactive parent is different: when the runtime lacks an atomic idempotent `continue-if-idle`/native async-rewake primitive, hooks must not simulate one by blocking Stop, parking every tool, or injecting a synthetic user turn. The parent remains conversational and the registered child is explicitly operator-driven fallback; terminal/quiescence/readiness and cleanup still fail closed independently. Legacy adapter-specific receipts may be consumed only by exact terminal typed harvest. Runtime-native subagents are a separate surface. | Probe the runtime completion surface before launch. Use only a session supervisor owned by the parent runtime (Codex App Server for Codex headless owners, Claude resume for Claude owners). A runtime without atomic idle claim and idempotent wake reports explicit fallback and must not claim interactive auto-resume. Approval requests remain owned by the interactive client; a completion sidecar must never subscribe as approval authority. |
