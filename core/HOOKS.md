...
| registered-child parent park | adapter-native pre-tool policy + `utilities/dispatch-wait.sh` | `adapter-coupled-automation` | While an exact registered child row is open, keep its depth-0 parent session or depth-1 conductor out of the model/tool loop. Permit only a long exact-attempt wait, continuation of that same wait, and typed exact-attempt harvest; deny raw transcript/log/artifact/source/git inspection, redundant liveness, parallel work, and shell composition. Resume only after the exact row closes. | Use a native all-local-tools pre-hook or an equally strong tool allowlist keyed by `parent_sid` / `parent_attempt_id`. If hosted or specialized tools bypass interception, disclose that boundary and retain the prompt plus conformance fallback; never claim total enforcement. |
