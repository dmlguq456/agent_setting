...
| conductor Stop gate | `hooks/conductor-stop-gate.sh` | `portable-check` (**UNREGISTERED — fallback only**) | Legacy SD-14b fallback for a runtime without either SD-78 session supervisor. It blocks a conductor turn-end with open children and directs exact `dispatch-wait`/harvest recovery. **Held / not registered** because Claude print-mode Stop does not preserve a reliable result stream. | Use only for an explicitly reported polling fallback; never register it as a normal completion path. Codex's trusted native Stop bridge below is adapter-owned and does not activate this script. |
| registered-child completion park | adapter session supervisor or trusted Codex Stop + native pre-tool policy | `adapter-coupled-automation` | A standard+ owner yields after registering a batch; its headless supervisor joins the exact `parent_attempt_id` batch outside the model and resumes the same session once with a bounded typed receipt. A direct interactive Codex parent instead ends its turn after starting a stamped depth-1 child; successful spawn first binds the exact attempt to hashed pending state, so trusted Stop follows the immutable set across `open`→`done`, joins without model/tool calls, and emits one `decision=block` continuation when exact harvest is ready. Atomic attempt- or hashed-session-scoped phase state permits only the actions for that delivery phase: headless owners may add a checked sibling before delivery, interactive parents admit no tool before delivery, and both admit only exact harvest afterward. A valid exact harvest consumes its receipt and the last receipt removes session state. Model-issued waits, raw inspection, liveness, unrelated work, and shell composition are denied. Current-hash Stop/PreToolUse trust is proved before an interactive process is claimed or spawned; a `launch_claimed=0` registration is never a park candidate. An explicit polling fallback preserves the same terminal-nonquiescent park. The post-exit watcher reconciles every exact owner PID termination, including model/auth/limit failure before any child exists; an `open` registry row alone is never liveness evidence. Unstamped/legacy interactive rows retain the polling fallback, and terminal live/unverifiable rows remain fail-closed for readiness and cleanup without becoming a session-wide local-tool lock. Runtime-native subagents are a separate surface and create no registered park candidate. | Probe the runtime completion surface before launch and validate Codex trust through its authoritative current-hash hook listing. Realize phase policy in the runtime's native pre-tool surface without mutating user configuration. If unavailable, fail before a native-Stop spawn or report the checked `dispatch-wait` fallback explicitly; arbitrary detached output never claims auto-resume. |
