...
#   (autopilot-lab references/eval-procedure.md L87·L111: audio→HTML — md 는 <audio> 차단, figure 만이면 md 인라인). craft.
# hard: experiments/ 하위 report HTML 에 <audio> 태그 존재.
#   ※ lab 은 실험을 전용 worktree(repo-wt/<slug>) 의 exp/ 브랜치에 격리 커밋한다(OPERATIONS §5.10).
...
if [ -n "$htmlaudio" ]; then
  ...
  echo "FAIL: 오디오 결과인데 <audio> 재생 HTML 없음 — audio→HTML 관습 누락 (references/eval-procedure.md L111)"; fail=1
fi
