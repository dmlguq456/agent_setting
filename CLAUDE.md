...
`autopilot-code` / `autopilot-doc` 의 `--user-refine` 같은 pause 옵션은 **사용자가 명시적으로 입력했을 때만** 켠다. 사용자가 "신중히 진행해 줘" / "한 번 봐줘" / camera-ready / submission 직전 같은 high-stakes 신호를 줬다고 해서, 또는 task 가 복잡하다는 이유로, 메인 Claude 가 args 구성 단계에서 임의로 `--user-refine` 을 추가하지 않는다.
...
### §5. 질문에 사용자가 답하지 않으면 추천 방향으로 자율 진행 (하네스 강제 — 턴 단위)

**하네스 한계 인정**: 메인 Claude 는 _turn 단위_ 로만 깨어난다 — _시간 측정_ 직접 못함. _N 분 같은 timer_ 는 `ScheduleWakeup` 도구 또는 `/loop` skill 안에서만 작동 (일반 turn 에서는 X).

**따라서 §5 는 두 layer 로 강제**:

#### Layer 1 — 턴 단위 강제 (default, _모든_ 질문)

질문을 던진 직후 _다음_ 사용자 메시지가 _본 질문에 대한 답이 아니면_ 즉시 추천 방향 자율 진행. 시간 측정 없이 _다음 메시지 시점_ 이 사실상 마감. 다음 세 케이스 모두 포함:

1. 사용자 응답이 다른 주제로 옴 (일반 코멘트·짧은 동의 표시만·새로운 명령)
2. 사용자가 질문을 **못 보고 그냥 다음 메시지로 넘어감** (질문을 시야에서 놓치는 흔한 경우)
3. 사용자 응답 자체가 없는 채로 시간이 지남 — 다음 메시지가 _언제든_ 도착하는 시점에 같은 처리
...
**질문 던질 때 명시 의무** — 응답 끝에 한 줄로 안내:

> _이 질문에 답을 안 주시면 추천 방향 (X) 으로 다음 메시지 시점에 자율 진행._

이게 _하네스 강제_ 의 명시 형태. 사용자가 _시간 제한_ 을 직접 본 상태에서 _답 안 주기로 선택_ 하는 것도 _기본 진행_ 의 동의로 해석.

#### Layer 2 — 시간 기반 polling (사용자가 명시할 때)

_명시적 시간 단위 자동 진행_ 이 필요한 자리 (장기간 polling / 외부 작업 대기) 에서는 메인 Claude 가 자체 시간 측정 X. 사용자가 `/loop <interval> <prompt>` 또는 `/schedule` 명시해야 `ScheduleWakeup` 이 발동되어 _N 분 후 자동 깨움_ 가능.

메인 Claude 의 역할 — _시간 기반 polling 이 필요해 보이는 자리에서 사용자에게_ `/loop` 또는 `/schedule` 사용 권유. 단순 _다음 사용자 메시지를 기다리면 됨_ 한 자리에서는 권유 안 함 (Layer 1 으로 충분).
...
- 본 파일을 업데이트할 시점: (a) source-of-truth 위치가 바뀔 때, (b) artifact_dir 컨벤션이 바뀔 때, (c) scope 경계가 근본적으로 변경될 때, (d) **도메인 트리거 표에 새 행 추가/제거**할 때, (e) **응답 원칙 (§1~§5) 이 추가·변경**될 때. 그 외엔 README.md만 sync.
