...
- **사용자 메모리**: `~/.claude/memory/memory.db`(SQLite 진실원천, 전용 repo `claude-memory` — `dump.jsonl` 텍스트 mirror 추적, config repo 에선 gitignore) · 세션 주입 source · 하네스 write 면 `~/.claude/projects/<cwd>/memory/`(SessionEnd `mem sync` 로 DB durable mirror)
- **사용자 프로필**: DB `memory.db` `type=profile` 레코드 (`mem profile <aspect>` 로 읽음; `user_profile/` 디렉토리는 제거됨 — on-demand `mem export --target profile` 캐시만) — cross-project 사용자 산출물 패턴 (figure / writing / presentation / analysis / domain / coding)
...
| **과거 결정·교정·컨벤션 회상 필요** (이 프로젝트의 _왜 이렇게 정했나·전에 어떻게 합의했나_ 류인데 세션 시작 주입 메모리·인덱스로 안 풀릴 때) | `~/.claude/tools/memory/recall.sh "<query>"` (읽기 전용; 현 cwd 기본 / `--all`=cross-cwd / `--sessions`=raw 대화까지) | **Claude 가 직접 실행 — 사용자가 쉘 칠 일 없음.** recall.sh = `mem recall` thin wrapper(store FTS5 색인 + LIKE/rg fallback). 정제 메모리 먼저, 안 나오면 `--sessions`. 매 턴 X, 필요 자리만(token 절약). 결과는 live 코드로 교차검증(메모리=작성시점 진실, stale 가능). `index-check.sh` = *legacy `projects/<cwd>/memory/` MEMORY.md 텍스트 인덱스 전용*(store 색인은 `mem index`). 상세 = CONVENTIONS §7.4. |
