...
2. **ceremony 작은 3 개** (`audit` / `notes` / `analyze-project`) → 컨펌 없이 즉시 invoke
...
autopilot-code dev 모드로 X 를 Y 하게 진행 (qa thorough, user-refine off)
  ↳ task: "..."
  ↳ 근거: cwd 가 plan 폴더 + 최근 dev_log → debug 아닌 dev
```
...
- **Notion 가이드**: `~/.claude/notion_guide.md`
...
| 트리거 | 자동 참조 | 비고 |
|---|---|---|
| **Notion 작업** | `~/.claude/notion_guide.md` | 메인 Claude 가 `mcp__claude_ai_Notion__*` 직접 호출. sub-agent 위임 X (MCP 접근 제약). |
...
| **세션 시작 / 새 cwd 진입** | `<cwd>/.claude_reports/NOTES.md` (있을 때만) | `/notes` 명령으로만 갱신 (자동 X). |
| **사용자 향 산출물 wording 작성·수정** (paper / strategy / report / 발표 / 노션 / README) | `~/.claude/agents/editorial-team.md` | 변경 직후 같은 turn 안 `Agent(편집팀)` _다듬기 모드_ 호출 의무. **트리거 X** — Claude instruction 파일 (CLAUDE.md / SKILL.md / agents/*.md / CONVENTIONS / DESIGN_PRINCIPLES / notion_guide) 자체. |
