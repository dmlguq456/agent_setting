...
각 skill 내부의 사용자 질문 단계 (예: analyze-project mode clarification, autopilot-draft Step 0 / autopilot-research Step 1.5 Scope Clarification, autopilot-refine artifact 식별 ask, audit fuzzy match 다중 후보 ask, autopilot-refine `--confirm` chat-pause) 에서도 본 §5 가 자동 적용. skill 본문이 timeout 안내를 명시하지 않아도 메인 Claude 는 질문 던질 때 ScheduleWakeup 동시 호출 의무.

답 없으면 추천 방향 자율 진행 (skill 내부 _default_ 또는 _첫 옵션_).
