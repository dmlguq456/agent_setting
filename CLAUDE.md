...
| **당직 보고 처리 발화** (`당직 처리` / `당직 보고` / `scout 처리` — alias 동일) | `/home/nas/user/Uihyeop/notes/scout/` 최신 보고 | 보고 Read → 발견별 triage 제안 → 승인된 것 실행. 기계적 정리(worktree remove 등)는 확인 후 즉시, 파괴 급은 별도 confirm. 처리한 발견은 보고 파일에 ✅ 표시. 루프 호칭: 당직=scout·서무=note·모의훈련=golden·연수=study (`loops/README.md`). |
