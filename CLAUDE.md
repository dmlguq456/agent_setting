...
### §5. 질문에 사용자가 답하지 않으면 추천 방향으로 자율 진행

사용자에게 선택지 (`A 인가요, B 인가요?` / `어느 쪽으로 갈까요?`) 를 물었는데 **다음 사용자 메시지가 그 질문에 대한 답이 아닌** 경우 — 다음 세 가지를 모두 포함:

1. 사용자 응답이 다른 주제로 옴 (일반 코멘트·짧은 동의 표시만·새로운 명령)
2. 사용자가 질문을 **못 보고 그냥 다음 메시지로 넘어감** (질문을 시야에서 놓치는 흔한 경우 — 사용자가 모호하게 답한 게 아니라 _답이 없는_ 케이스)
3. 사용자 응답 자체가 없는 채로 시간이 지남

위 세 경우 모두 메인 Claude 는 같은 동작:
