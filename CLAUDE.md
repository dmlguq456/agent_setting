...
### §0. [최상단 규칙] spec-backed 프로젝트 — 파이프 우선

다른 모든 원칙에 앞서는 최우선 규칙. **cwd 또는 상위에 `.claude_reports/spec/<project>/pipeline_state.yaml` 이 있으면** (새 세션 포함), 수정·기능 요청을 _ad-hoc 직접 진단 + Edit 으로 끝내지 않는다._ 순서:

1. **기존 산출물 파악** (손대기 전) — `spec/<project>/prd.md` · `pipeline_state.yaml` · 최근 `plans/<project>/*`.
2. **spec-drift 사전 체크** — spec-significant (route / schema·entity / UI-flow / 외부 연동 / 마이그레이션) 면 **`autopilot-spec` update** (prd.md 갱신 + `_internal/versions/v{N}/` snapshot). drift 명확하면 자율 진행, _애매하면 사용자 확인_. spec 자체가 없으면 `autopilot-spec` 먼저 (throwaway 1 회성만 예외).
3. **`autopilot-code --qa quick` 경유** — 작은 요청도 파이프로 산출물 (plan/log) 남김 → `plans/<project>/`.

직접 Edit 은 _순수 typo·1 줄 포맷_ 만. 상세 흐름 = `WORKFLOW.md` §7. (SessionStart hook 이 새 세션마다 이 규칙을 리마인드 — 강제 아님.)

### §1. 말투 (가벼운 선호 — 어겨도 치명적이지 않음)

한국어 응답에 영어 일반 명사·동사구를 어순에 박지 않는다 (LaTeX·경로·논문/학회/모델/지표·정착 외래어는 영어 그대로). 한 응답 안 같은 개념은 같은 표기로 통일. 어미: chat 기본 해요체, 보고서·짧은 메타 라벨만 평어·개조식, 친절 안내체 (`~해 드릴게요 / ~가 좋겠습니다`) 회피.
...
> (spec-backed 사후 수정 규칙은 최상단 **§0** 으로 승격 — 본 자리 중복 제거.)
...
| **spec-backed cwd / 사후 수정 요청** (cwd·상위에 `.claude_reports/spec/<project>/pipeline_state.yaml` 존재) | 본 문서 **§0** + WORKFLOW §7 | 최상단 규칙 — 기존 산출물 파악 → spec-drift 체크 (autopilot-spec update) → autopilot-code 파이프. ad-hoc 직접 Edit 은 순수 typo 만. |
...
- 업데이트 시점: (a) source-of-truth 위치 변경 (b) artifact_dir 변경 (c) scope 경계 변경 (d) 도메인 트리거 행 추가/제거 (e) 응답 원칙 §0~§8 추가/변경.
