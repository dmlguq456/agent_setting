...
| **과거 결정·교정·컨벤션 회상 필요** (이 프로젝트의 _왜 이렇게 정했나·전에 어떻게 합의했나_ 류인데 세션 시작 주입 메모리·인덱스로 안 풀릴 때) | `~/.claude/tools/memory/recall.sh "<query>"` (읽기 전용; 현 cwd 기본 / `--all`=cross-cwd) | **Claude 가 직접 실행 — 사용자가 쉘 칠 일 없음.** 매 턴 X, 필요 자리만(token 절약). 결과는 live 코드로 교차검증(메모리=작성시점 진실, stale 가능). 인덱스 점검 `index-check.sh` + 상세 = CONVENTIONS §7.4. |
